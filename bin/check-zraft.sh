...
# IMPLEMENTATION_MODULE=${MODULE} bin/partisan-analysis.escript _checkouts/zraft_lib/src/zraft_consensus.erl

# echo "Performing static analaysis [build] ..."
# IMPLEMENTATION_MODULE=${MODULE} bin/partisan-analysis.escript _build/default/lib/zraft_lib/src/zraft_consensus.erl
...
# rm -rf priv/lager; pkill -9 beam.smp; DISABLE_RANDOM=true RESTART_NODES=false IMPLEMENTATION_MODULE=${MODULE} NUM_TESTS=10 SCHEDULER=finite_fault bin/counterexample-find.sh

echo "Running multi run fault-injector..."
rm -rf priv/lager; pkill -9 beam.smp; DISABLE_RANDOM=true RESTART_NODES=false IMPLEMENTATION_MODULE=${MODULE} NUM_TESTS=100 SCHEDULER=finite_fault FAULT_INJECTION=true bin/counterexample-find.sh