# PDDR-000004: Thicket Engine — Faithful Implementation, Seam Revision, and Property-Based Validation

- **Status:** PROPOSED
- **Decision:** When a measured interior-node-load imbalance justifies it (the gate
  from PDDR-000002), implement **Thicket** (Ferreira, Leitão & Rodrigues, SRDS 2010)
...
PDDR-000002 made the tree engine a per-group slot and accepted Thicket as the gated
...
- **Load-biased Plumtree** (bias eager selection by load, no disjointness). Rejected in
  PDDR-000002 and reaffirmed: it muddies one engine with two behaviours and has no
...
Thicket is realized as `partisan_thicket_engine`: a pure, I/O-free module implementing
the paper's Algorithms 1–4 and returning `{State, [action()]}`, so the whole protocol
is exercised in a deterministic simulation. It runs behind `partisan_broadcast_engine`
in raw-dispatch mode — the shell hands it whole wire messages and executes its `send`,
`deliver` and `fetch` actions against the group's single handler — and is selected per
group with `engine => partisan_thicket_engine`, off by default.

`prop_partisan_thicket_engine` is the primary validator. It instantiates N engine
states over a mesh, randomizes both message-delivery and tick order, drives each
configuration to quiescence, and asserts the two load-bearing invariants:

- **Interior-load bound** — no node is interior in more than `maxLoad` trees, for
  every tree count, fanout, ordering and cap.
- **Coverage** — every node delivers every broadcast when `maxLoad ≥ T`, over up to
  four divergent trees. That range necessarily includes `T > f`, where
  interior-node-disjoint trees cannot fit and coverage rests instead on §4.4
  link-reassignment (one overlay link may serve several trees) and on branching into
  the headroom `maxLoad` allows. Binding caps (`maxLoad < T`) and larger tree counts
  hold the load bound but not coverage, and lie outside the paper's guidance (keep
  `maxLoad` above the tree count, `T ≤ f`).

Two properties keep the engine's state bounded: it holds no payloads (only delivered
message ids, delegating payload storage and re-supply to the handler, as Plumtree
does), and its announcement set — which doubles as the repair server-directory — is
bounded by an age-based window rather than growing with the message count.

Enabling Thicket for a real group stays gated on a measured interior-node-load
imbalance, the gate from PDDR-000002.

## Related records

- **PDDR-000002** — made the tree engine a per-group pluggable slot and accepted
