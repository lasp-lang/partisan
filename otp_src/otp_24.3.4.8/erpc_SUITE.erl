...
call_test(Config) ->
    ...
    try
        partisan_erpc:call(Node, erlang, node, []),
        ct:fail(unexpected)
    catch
        error:{erpc, noconnection} ->
            ok
    ...
    try
        partisan_erpc:call(badnodename, erlang, node, []),
        ct:fail(unexpected)
    catch
        error:{erpc, noconnection} ->
            ok
...
call_test(Node, Timeout) ->
    ...
    case {partisan:node() == Node, Timeout == infinity} of
        {true, true} ->
...
call_reqtmo(Config) when is_list(Config) ->
    Fun = fun (Node, SendMe, Timeout) ->
                  try
                      partisan_erpc:call(Node, partisan, send,
...
cast(Config) when is_list(Config) ->
    %% silently fail
    ok = partisan_erpc:cast(badnodename, partisan, send, [hej]),

    try
        partisan_erpc:cast(<<"node">>, partisan, send, [hej]),
        ct:fail(unexpected)
    catch
        error:{erpc, badarg} -> ok
    end,
    try
        partisan_erpc:cast(partisan:node(), partisan, send, hej),
        ct:fail(unexpected)
    catch
        error:{erpc, badarg} -> ok
    end,
    ...
    partisan_erpc:cast(partisan:node(), partisan, send, [partisan:self()|blupp]), %% silent remote error...

    Me = partisan:self(),
    Ok1 = partisan:make_ref(),
    ok = partisan_erpc:cast(partisan:node(), partisan, send, [Me, {mfa, Ok1}]),
    receive
        {mfa, Ok1} -> ok
    end,
    ...
    ok = partisan_erpc:cast(Node, partisan, send, [Me, {mfa, Ok2}]),
    receive
        {mfa, Ok2} -> ok
    end,
    ...
    ok = partisan_erpc:cast(Node, partisan, send, [Me, wont_reach_me]),
...
send_request(Config) when is_list(Config) ->
    ...
    ReqId6 = partisan_erpc:send_request(Node4, erlang, node, []),
    ...
    ReqId8 = partisan_erpc:send_request(Node4, erlang, node, []),
    receive
        Msg2 ->
            try
    ...
    case start_22_node(Config) of
        {ok, Node5} ->
            ReqId9 = partisan_erpc:send_request(Node5, erlang, node, []),
            try
...
send_request_receive_reqtmo(Config) when is_list(Config) ->
    Fun = fun (Node, SendMe, Timeout) ->
                  RID = partisan_erpc:send_request(Node, partisan, send,
...
send_request_against_old_node(Config) when is_list(Config) ->
    case start_22_node(Config) of
        {ok, Node22} ->
            RID1 = partisan_erpc:send_request(Node22, erlang, node, []),
            RID2 = partisan_erpc:send_request(Node22, erlang, node, []),
            RID3 = partisan_erpc:send_request(Node22, erlang, node, []),
            try
...
multicall(Config) ->
    ...
    [{ok, ThisNode},
     ...
     {ok, Node5},
     {error, {erpc, noconnection}}]
        = partisan_erpc:multicall(Nodes ++ [badnodename], erlang, node, []),
    ...
    try
        partisan_erpc:multicall([Node1, Node2, Node3, Node4, Node5 | node()], erlang, node, []),
        ct:fail(unexpected)
    catch
        error:{erpc, badarg} ->
            ok
    ...
    [{ok, ThisNode},
     ...
     {ok, Node4},
     {ok, Node5}]
        = partisan_erpc:multicall(Nodes, erlang, node, []),
    ...
    [{ok, ThisNode},
     ...
     {ok, Node4},
     {ok, Node5}]
        = partisan_erpc:multicall(Nodes, erlang, node, [], 60000),
    ...
    [{ok, ThisNode},
     ...
     {ok, Node4},
     {ok, Node5}]
        = partisan_erpc:multicall(Nodes++Nodes, erlang, node, []),
    ...
    [{ok, ThisNode},
     ...
     {ok, Node4},
     {ok, Node5}]
        = partisan_erpc:multicall(Nodes++Nodes, erlang, node, [], 60000),
...
multicast(Config) when is_list(Config) ->
    ...
    Nodes = [node(), Node],
    try
        partisan_erpc:multicast(Nodes, partisan, send, hej),
        ct:fail(unexpected)
    catch
        error:{erpc, badarg} -> ok
    end,
    try
        partisan_erpc:multicast(node(), partisan, send, [hej]),
        ct:fail(unexpected)
    catch
        error:{erpc, badarg} -> ok
    end,
    try
        partisan_erpc:multicast([<<"node">>, Node], partisan, send, [hej]),
        ct:fail(unexpected)
    catch
        error:{erpc, badarg} -> ok
    end,
    ...
    try
        partisan_erpc:multicast(Nodes, partisan, partisan:make_ref(), [hej]),
        ct:fail(unexpected)
    catch
        error:{erpc, badarg} -> ok
    end,
    ...
    %% Silently fail...
    partisan_erpc:multicast([badnodename], partisan, send, [partisan:self(), blupp]),
    %% silent remote error...
    partisan_erpc:multicast(Nodes, partisan, send, [partisan:self()|blupp]),

    Me = partisan:self(),
    Ok1 = partisan:make_ref(),
    ok = partisan_erpc:multicast(Nodes, partisan, send, [Me, {mfa, Ok1}]),
    receive
        {mfa, Ok1} -> ok
    end,
    receive
    ...
    ok = partisan_erpc:multicast([Node], partisan, send, [Me, wont_reach_me]),
...
timeout_limit(Config) when is_list(Config) ->
    ...
    Node = partisan_erpc:call(Node, erlang, node, [], MaxTmo),
    try
        partisan_erpc:call(node(), erlang, node, [], MaxTmo+1),
        ct:fail(unexpected)
    catch
        error:{erpc, badarg} ->
            ok
    end,
    [{ok,Node}] = partisan_erpc:multicall([Node], erlang, node, [], MaxTmo),
    try
        partisan_erpc:multicall([Node], erlang, node, [], MaxTmo+1),
        ct:fail(unexpected)
    catch
        error:{erpc, badarg} ->
            ok
    end,
    ReqId1 = partisan_erpc:send_request(Node, erlang, node, []),
    Node = partisan_erpc:receive_response(ReqId1, MaxTmo),
    ReqId2 = partisan_erpc:send_request(Node, erlang, node, []),
    try
    ...
    ReqId3 = partisan_erpc:send_request(Node, erlang, node, []),
    Node = partisan_erpc:receive_response(ReqId3, MaxTmo),
    ReqId4 = partisan_erpc:send_request(Node, erlang, node, []),
    try
