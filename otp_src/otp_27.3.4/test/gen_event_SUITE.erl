...
start(Config) when is_list(Config) ->
    ...
    %anonymous
    {ok, {Pid1b, Mon1b}} = gen_event:start_monitor(),
    ...
    {ok, {Pid5b, Mon5b}} = gen_event:start_monitor(
    ...
    {error, {already_started, _}} = gen_event:start_link(
        {via, dummy_via, my_dummy_name}
    ),
    {error, {already_started, _}} = gen_event:start(
        {via, dummy_via, my_dummy_name}
    ),
    ...
    {ok, {Pid7b, Mon7b}} = gen_event:start_monitor(
        {via, dummy_via, my_dummy_name}
    ),
    {error, {already_started, _}} = gen_event:start_monitor(
        {via, dummy_via, my_dummy_name}
    ),
    {error, {already_started, _}} = gen_event:start(
        {via, dummy_via, my_dummy_name}
...
start_opt(Config) when is_list(Config) ->
    ...
    %% test spawn_opt
    MinHeapSz = 10000,
    {ok, Pid9} = gen_event:start_link(?LMGR, [
...
auto_hibernate(Config) when is_list(Config) ->
    ...
    {ok, Pid} = gen_event:start({local, auto_hibernate_handler}, [
...
call(Config) when is_list(Config) ->
    ...
    {reply, {ok, hejhopp}} =
        receive
            Msg -> gen_event:check_response(Msg, P2)
        after 1000 -> exit(tmo)
        end,

    ok = gen_event:delete_handler(my_dummy_handler, {dummy_h, 1}, []),
    {ok, swapped} = gen_event:call(
        my_dummy_handler,
        dummy_h,
...
format_log_2(_Config) ->
    ...
    WL1 = length(WStr1),
    WExpected1 =
        "** Undefined handle_info in gen_event_SUITE\n"
...
send_request_receive_reqid_collection(Pid1, Pid2, Pid3) ->
    ...
    ReqIdC2 = gen_event:send_request(
        Pid2, dummy_h, {delayed_answer, 1}, req2, ReqIdC1
    ),
    2 = gen_event:reqids_size(ReqIdC2),

    ReqIdC3 = gen_event:send_request(
        Pid3, dummy_h, {delayed_answer, 200}, req3, ReqIdC2
    ),
    3 = gen_event:reqids_size(ReqIdC3),

    {{reply, delayed}, req2, ReqIdC4} = gen_event:receive_response(
        ReqIdC3, infinity, true
    ),
    2 = gen_event:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC5} = gen_event:receive_response(
        ReqIdC4, 5678, true
    ),
    1 = gen_event:reqids_size(ReqIdC5),

    {{reply, delayed}, req1, ReqIdC6} = gen_event:receive_response(
        ReqIdC5, 5000, true
    ),
...
send_request_receive_reqid_collection_error(Pid1, Pid2, Pid3) ->
    ...
    {{error, {noproc, _}}, req2, ReqIdC5} = gen_event:receive_response(
        ReqIdC4, 2000, true
    ),
    3 = gen_event:reqids_size(ReqIdC5),

    {{reply, delayed}, req3, ReqIdC5} = gen_event:receive_response(
        ReqIdC5, infinity, false
    ),
    {{reply, delayed}, req1, ReqIdC5} = gen_event:receive_response(
        ReqIdC5, infinity, false
    ),
    {{error, bad_module}, req4, ReqIdC5} = gen_event:wait_response(
        ReqIdC5, infinity, false
    ),
...
send_request_wait_reqid_collection(Pid1, Pid2, Pid3) ->
    ...
    {{reply, delayed}, req2, ReqIdC4} = gen_event:wait_response(
        ReqIdC3, infinity, true
    ),
    2 = gen_event:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC5} = gen_event:wait_response(
        ReqIdC4, 5678, true
    ),
    1 = gen_event:reqids_size(ReqIdC5),

    {{reply, delayed}, req1, ReqIdC6} = gen_event:wait_response(
        ReqIdC5, 5000, true
    ),
    0 = gen_event:reqids_size(ReqIdC6),
...
send_request_wait_reqid_collection_timeout(Pid1, Pid2, Pid3) ->
    ...
    {{reply, delayed}, req3, ReqIdC4} = gen_event:wait_response(
        ReqIdC4, {abs, Deadline + 1500}, false
    ),
    {{reply, delayed}, req1, ReqIdC4} = gen_event:wait_response(
        ReqIdC4, {abs, Deadline + 1500}, false
    ),
...
send_request_wait_reqid_collection_error(Pid1, Pid2, Pid3) ->
    ...
    {{error, {noproc, _}}, req2, ReqIdC5} = gen_event:wait_response(
        ReqIdC4, 2000, true
    ),
    3 = gen_event:reqids_size(ReqIdC5),
    ...
    {{reply, delayed}, req1, ReqIdC5} = gen_event:wait_response(
        ReqIdC5, infinity, false
    ),
    {{error, bad_module}, req4, ReqIdC5} = gen_event:wait_response(
        ReqIdC5, infinity, false
...
send_request_check_reqid_collection(Pid1, Pid2, Pid3) ->
    ...
    ReqIdC1 = gen_event:send_request(
        Pid1, dummy_h, {delayed_answer, 400}, req1, ReqIdC0
    ),
    1 = gen_event:reqids_size(ReqIdC1),
    ...
    {{reply, delayed}, req2, ReqIdC4} = gen_event:check_response(
        next_msg(), ReqIdC3, true
    ),
    2 = gen_event:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC5} = gen_event:check_response(
        next_msg(), ReqIdC4, true
    ),
    1 = gen_event:reqids_size(ReqIdC5),

    {{reply, delayed}, req1, ReqIdC6} = gen_event:check_response(
...
send_request_check_reqid_collection_error(Pid1, Pid2, Pid3) ->
    ...
    {{error, {noproc, _}}, req2, ReqIdC5} = gen_event:check_response(
        next_msg(), ReqIdC4, true
    ),
    3 = gen_event:reqids_size(ReqIdC5),

    {{reply, delayed}, req3, ReqIdC5} = gen_event:check_response(
        next_msg(), ReqIdC5, false
    ),
    {{reply, delayed}, req1, ReqIdC5} = gen_event:check_response(
        next_msg(), ReqIdC5, false
    ),
    {{error, bad_module}, req4, ReqIdC5} = gen_event:check_response(
        next_msg(), ReqIdC5, false
    ),
