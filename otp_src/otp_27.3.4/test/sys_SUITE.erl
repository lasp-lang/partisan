...
log_to_file(Config) when is_list(Config) ->
    TempName = test_server:temp_name(
        proplists:get_value(priv_dir, Config) ++ "sys."
    ),
    {ok, _Server} = start(),
...
install(Config) when is_list(Config) ->
    ...
    [
        ...
        {spy_got, {request, 4}, sys_SUITE_server}
    ] = get_messages(),
    ...
    ok =
        case catch sys:get_state(Mod) of
            [] ->
                ok;
            {'EXIT', {
                {callback_failed, {Mod, system_get_state}, {throw, fail}}, _
            }} ->
                ok
        end,
    ...
    ok =
        case catch sys:replace_state(Mod, fun(_) -> {} end) of
            {} ->
                ok;
            {'EXIT', {
                {callback_failed, {Mod, system_replace_state}, {throw, fail}}, _
            }} ->
                ok
        end,
    ok = sync_terminate(Mod),
    ...
    ok =
        case catch sys:replace_state(Mod, StateFun) of
            {} ->
                ok;
            {'EXIT', {
                {callback_failed, {Mod, system_replace_state}, {error, fail}}, _
            }} ->
                ok;
