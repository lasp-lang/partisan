...
-export([
    start/1,
    start_event_timeout/1,
    start_event_timeout_zero/1,
    crash/1,
    loop_start_fail/1,
    call/1,
    send_request/1,
    ...
    multicall_down/1,
    multicall_remote/1,
    multicall_remote_old1/1,
    multicall_remote_old2/1,
    ...
    crash_in_format_status/1,
    throw_in_format_status/1,
    format_all_status/1,
    get_state/1,
    replace_state/1,
...
groups() ->
    [
        ...
        {multi_call, [], [
            {group, multi_call_parallel}, {group, multi_call_sequence}
        ]},
        {multi_call_parallel, [parallel], [
            multicall,
            multicall_down,
            multicall_remote,
            multicall_remote_old1,
            multicall_remote_old2
        ]},
        {multi_call_sequence, [], [
            multicall_recv_opt_success,
            multicall_recv_opt_timeout,
        ...
        {format_status, [], [
            ...
            crash_in_format_status,
            throw_in_format_status,
            format_all_status
        ]},
        {undef_callbacks, [], [
            undef_handle_call,
...
start(Config) when is_list(Config) ->
    ...
    {error, timeout} = gen_server:start(
        gen_server_SUITE,
        sleep,
        [{timeout, 100}]
    ),
...
start_event_timeout(Config) when is_list(Config) ->
    ...
    lists:foreach(
        ...
        [
            {StartFun, Arg, Interrupt}
         || StartFun <- [
                fun(X) -> start_link(spec_init_action, [[], X]) end,
...
loop_start_fail(Config, [{Start, Opts} | Start_Opts]) ->
    loop_start_fail(
        ...
        {ets, {return, {stop, failed_to_start}}},
        Opts,
    ...
    loop_start_fail(
        fun gen_server:Start/3,
        {ets, {crash, error, bailout}},
        Opts,
        fun({error, {bailout, ST}}) when is_list(ST) -> ok end
    ),
    loop_start_fail(
        fun gen_server:Start/3,
        {ets, {crash, exit, bailout}},
        Opts,
...
send_request(Config) when is_list(Config) ->
    ...
    receive
        {[alias | Ref], _} = Msg when is_reference(Ref) ->
    ...
    {ok, RPid} = rpc:call(Remote, gen_server, start, [
    ...
    {ok, _} = rpc:call(Remote, gen_server, start, [
        {local, remote}, ?MODULE, [], []
    ]),
...
send_request_receive_reqid_collection(Config) when is_list(Config) ->
    {ok, Pid1} = gen_server:start_link(
        {local, my_test_name1},
        gen_server_SUITE,
        [],
        []
    ),
    {ok, Pid2} = gen_server:start_link(
        {local, my_test_name2},
        gen_server_SUITE,
        [],
        []
    ),
    {ok, Pid3} = gen_server:start_link(
        {local, my_test_name3},
        gen_server_SUITE,
        [],
        []
...
send_request_receive_reqid_collection(Pid1, Pid2, Pid3) ->
    ...
    ReqIdC3 = gen_server:send_request(
        Pid3, {delayed_answer, 200}, req3, ReqIdC2
    ),
    3 = gen_server:reqids_size(ReqIdC3),

    {{reply, delayed}, req2, ReqIdC4} = gen_server:receive_response(
        ReqIdC3, infinity, true
    ),
    2 = gen_server:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC5} = gen_server:receive_response(
        ReqIdC4, 5678, true
    ),
    1 = gen_server:reqids_size(ReqIdC5),

    {{reply, delayed}, req1, ReqIdC6} = gen_server:receive_response(
        ReqIdC5, 5000, true
    ),
...
send_request_receive_reqid_collection_timeout(Pid1, Pid2, Pid3) ->
    ...
    timeout = gen_server:receive_response(
        ReqIdC4, {abs, Deadline + 1000}, true
    ),
...
send_request_receive_reqid_collection_error(Pid1, Pid2, Pid3) ->
    ...
    {{error, {shutdown, _}}, req2, ReqIdC4} = gen_server:receive_response(
        ReqIdC3, 2000, true
    ),
    2 = gen_server:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC4} = gen_server:receive_response(
        ReqIdC4, infinity, false
    ),

    {{reply, delayed}, req1, ReqIdC4} = gen_server:receive_response(
        ReqIdC4, infinity, false
    ),
...
send_request_wait_reqid_collection(Pid1, Pid2, Pid3) ->
    ...
    {{reply, delayed}, req2, ReqIdC4} = gen_server:wait_response(
        ReqIdC3, infinity, true
    ),
    2 = gen_server:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC5} = gen_server:wait_response(
        ReqIdC4, 5678, true
    ),
    1 = gen_server:reqids_size(ReqIdC5),

    {{reply, delayed}, req1, ReqIdC6} = gen_server:wait_response(
        ReqIdC5, 5000, true
    ),
    0 = gen_server:reqids_size(ReqIdC6),
...
send_request_wait_reqid_collection_timeout(Pid1, Pid2, Pid3) ->
    ...
    {{reply, delayed}, req3, ReqIdC4} = gen_server:wait_response(
        ReqIdC4, {abs, Deadline + 1500}, false
    ),
    {{reply, delayed}, req1, ReqIdC4} = gen_server:wait_response(
        ReqIdC4, {abs, Deadline + 1500}, false
    ),
...
send_request_wait_reqid_collection_error(Pid1, Pid2, Pid3) ->
    ...
    {{error, {shutdown, _}}, req2, ReqIdC4} = gen_server:wait_response(
        ReqIdC3, 2000, true
    ),
    2 = gen_server:reqids_size(ReqIdC4),
...
send_request_check_reqid_collection(Pid1, Pid2, Pid3) ->
    ...
    ReqIdC1 = gen_server:send_request(
        Pid1, {delayed_answer, 400}, req1, ReqIdC0
    ),
    1 = gen_server:reqids_size(ReqIdC1),
    ...
    {{reply, delayed}, req2, ReqIdC4} = gen_server:check_response(
        next_msg(), ReqIdC3, true
    ),
    2 = gen_server:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC5} = gen_server:check_response(
        next_msg(), ReqIdC4, true
    ),
    1 = gen_server:reqids_size(ReqIdC5),

    {{reply, delayed}, req1, ReqIdC6} = gen_server:check_response(
        next_msg(), ReqIdC5, true
    ),
    0 = gen_server:reqids_size(ReqIdC6),
...
send_request_check_reqid_collection_error(Pid1, Pid2, Pid3) ->
    ...
    {{error, {shutdown, _}}, req2, ReqIdC4} = gen_server:check_response(
        next_msg(), ReqIdC3, true
    ),
    2 = gen_server:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC4} = gen_server:check_response(
        next_msg(), ReqIdC4, false
    ),

    {{reply, delayed}, req1, ReqIdC4} = gen_server:check_response(
        next_msg(), ReqIdC4, false
    ),
...
continue(Config) when is_list(Config) ->
    ...
    [{Pid, before_continue}, {Pid, continue}, {Pid, after_continue}] = read_replies(
        Pid
    ),
...
calling_self(Config) when is_list(Config) ->
    ...
    {'EXIT', {calling_self, _}} =
        (catch gen_server:call(self(), oops, infinity)),
    ok.
...
multicall_down(Config) when is_list(Config) ->
    ...
    {Good, Bad} = gen_server:multi_call(
        [Name, node()],
        global_name_server,
        info,
        3000
...
multicall_remote_old_test(Config, OldN, Name) ->
    try
        ...
        {ok, ?MODULE, BeamCode} = erpc:call(hd(OldNodes), compile, file, [
            SrcFile, [binary]
        ]),
        LoadResult = lists:duplicate(length(OldNodes), {ok, {module, ?MODULE}}),
        LoadResult = erpc:multicall(OldNodes, code, load_binary, [
...
multicall_remote_test(
    ...
    Res3 = {[{Node1, delayed}, {ThisNode, delayed}], [Node2, Node3, Node4]},
...
multicall_recv_opt_test(Type) ->
    ...
    if
        Type == noconnection -> peer:stop(Peer2);
        true -> ok
    end,
    ...
    {Req, ExpRes, Tmo} =
        case Type of
            success ->
                {ping, {[{Node1, pong}, {Node2, pong}], []}, infinity};
            timeout ->
                {{delayed_answer, 100}, {[], Nodes}, 1};
            noconnection ->
                {ping, {[{Node1, pong}], [Node2]}, infinity}
        end,
    ...
    ct:log(
        "Time with empty message queue: ~p microsecond~n",
    ...
    ct:log(
        "Time with huge message queue: ~p microsecond~n",
...
do_otp_7669_stop() ->
    %% The name should never be registered after the return
    %% from gen_server:start/3.
    {error, stopped} = gen_server:start(
        {local, ?MODULE},
        ?MODULE,
        stop,
        []
...
call_format_status(Config) when is_list(Config) ->
    ...
    call_format_status(
        format_status_server, {data, [{"State", format_status_called}]}
    ),
    process_flag(trap_exit, OldFl).
...
crash_in_format_status(Config) when is_list(Config) ->
    ...
    crash_in_format_status(
        format_status_server, "format_status_server:format_status/1 crashed"
    ),
    process_flag(trap_exit, OldFl).
...
format_all_status(Config) when is_list(Config) ->
    ...
    {ok, Pid2} = gen_server:start_link(
        format_status_server, {state, State}, []
    ),
    catch gen_server:call(Pid2, crash),
    receive
        {'EXIT', Pid2, crashed} -> ok
    end,
    receive
        {error, _GroupLeader2,
...
replace_state(Config) when is_list(Config) ->
    ...
    {'EXIT', {
        {callback_failed, {gen_server, system_replace_state}, {throw, fail}}, _
    }} =
...
undef_handle_call(_Config) ->
    ...
    try
        ...
        exit:{
            {undef, [{oc_server, handle_call, _, _} | _]}, {gen_server, call, _}
        } ->
            ok
    end.
...
format_log_2(_Config) ->
    ...
    WL1 = length(WStr1),
    WExpected1 =
        "** Undefined handle_info in gen_server_SUITE\n"
        "** Unhandled message: ",
...
init({timeout, T, Pid, absolute}) ->
    {ok, [],
        {timeout, erlang:monotonic_time(millisecond) + T, {event_timeout, Pid},
            [{abs, true}]}};
...
init({hibernate, T, Pid, absolute}) ->
    {ok, [],
        {hibernate, erlang:monotonic_time(millisecond) + T,
            {event_timeout, Pid}, [{abs, true}]}};
...
handle_call({timeout, T, Pid, absolute}, _From, State) ->
    {reply, ok, State,
        {timeout, relative_absolute_timeout(T), {event_timeout, Pid}, [
            {abs, true}
        ]}};
...
handle_call({hibernate, T, Pid, absolute}, _From, State) ->
    {reply, ok, State,
        {hibernate, relative_absolute_timeout(T), {event_timeout, Pid}, [
            {abs, true}
        ]}};
...
old_release(N) ->
    OldRel = integer_to_list(
        list_to_integer(erlang:system_info(otp_release)) - N
    ),
