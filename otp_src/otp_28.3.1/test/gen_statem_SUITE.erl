...
start1(Config) ->
    ...
    {ok, {Pid1, Mon1}} = gen_statem:start_monitor(
...
error_format_status(Config) ->
    ...
    try
        error_format_status(
            Config, ?MODULE, {formatted, idle, "called format_status"}
        ),
        error_format_status(
            Config,
            format_status_statem,
...
terminate_crash_format(Config) ->
    ...
    try
        ...
        terminate_crash_format(
            [{callback_mode, handle_event_function} | Config],
            ?MODULE,
            {formatted, idle, crash_terminate},
...
format_all_status(Config) ->
    ...
    [
        ...
        {data, [{"State", {{state, idle}, {data, Data}}}]}
    ] = Info,
...
replace_state(Config) ->
    ...
    {callback_failed, {gen_statem, system_replace_state}, {error, fail}} =
...
hibernate(Config) ->
    ...
    wait_for_hibernated(Pid0, WaitHibernate),
    stop_it(Pid0),
    receive
        {'EXIT', Pid0, normal} -> ok
    after 5000 ->
    ...
    wait_for_hibernated(Pid, WaitHibernate),
    good_morning = gen_statem:call(Pid, wakeup_sync),
    is_not_hibernated(Pid, WaitHibernate),
    hibernating = gen_statem:call(Pid, hibernate_sync),
    wait_for_hibernated(Pid, WaitHibernate),
    ...
    wait_for_hibernated(Pid, WaitHibernate),
    sys:suspend(Pid),
    wait_for_hibernated(Pid, {erlang, hibernate, 3}, WaitHibernate),
    sys:resume(Pid),
    wait_for_hibernated(Pid, WaitHibernate),
    receive
    after WaitHibernate -> ok
    end,
    wait_for_hibernated(Pid, WaitHibernate),
...
auto_hibernate(Config) ->
    ...
    %% After init test
    is_not_hibernated(Pid, 2 * HibernateAfterTimeout),
    timer:sleep(HibernateAfterTimeout),
    wait_for_hibernated(Pid, 2 * HibernateAfterTimeout),
    %% After info test
    Pid ! {hping, self()},
    receive
        {Pid, hpong} ->
...
wait_for_hibernated(Pid, Time) ->
    wait_for_hibernated(Pid, {gen_statem, loop_hibernate, 3}, Time).

wait_for_hibernated(Pid, MFA, Time) ->
    receive
    after 1 -> ok
    end,
    wait_for_hibernated(Pid, MFA, Time, Time div 100).

wait_for_hibernated(Pid, MFA, Time, T) ->
    case erlang:process_info(Pid, current_function) of
        {current_function, MFA} ->
            ok;
        {current_function, WrongMFA} ->
            if
                Time =< 0 ->
                    ct:log("Wrong current_function: ~p\n", [WrongMFA]),
                    ct:fail(not_in_expected_current_function);
                true ->
                    receive
                    after T -> ok
                    end,
                    wait_for_hibernated(Pid, MFA, Time - T, T)
            end
    end.

is_not_hibernated(Pid, Time) ->
    is_not_hibernated(Pid, {gen_statem, loop_hibernate, 3}, Time).

is_not_hibernated(Pid, MFA, Time) ->
    receive
    after 1 -> ok
    end,
    is_not_hibernated(Pid, MFA, Time, Time div 100).

is_not_hibernated(Pid, MFA, Time, T) ->
    case erlang:process_info(Pid, current_function) of
        {current_function, MFA} ->
            if
                Time =< 0 ->
                    ct:log("Wrong current_function: ~p\n", [MFA]),
                    ct:fail(still_in_unxpected_current_function);
                true ->
                    receive
                    after T -> ok
                    end,
                    is_not_hibernated(Pid, MFA, Time - T, T)
            end;
        {current_function, _} ->
            ok
    end.
...
enter_loop(_Config) ->
    ...
    after 1000 ->
        ct:fail(
            {gen_statem_started, process_info(self(), messages)}
        )
    end,
...
pop_too_many(_Config) ->
    ...
    % OTP-18239
    enter = gen_statem:call(STM, get_data),
    ...
    {{BadAction, _}, {gen_statem, call, [STM, pop_callback_module, infinity]}} =
...
send_request_receive_reqid_collection(Pid1, Pid2, Pid3) ->
    ...
    ReqIdC3 = gen_statem:send_request(
        Pid3, {delayed_answer, 200}, req3, ReqIdC2
    ),
    3 = gen_statem:reqids_size(ReqIdC3),

    {{reply, delayed}, req2, ReqIdC4} = gen_statem:receive_response(
        ReqIdC3, infinity, true
    ),
    2 = gen_statem:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC5} = gen_statem:receive_response(
        ReqIdC4, 5678, true
    ),
    1 = gen_statem:reqids_size(ReqIdC5),

    {{reply, delayed}, req1, ReqIdC6} = gen_statem:receive_response(
        ReqIdC5, 5000, true
    ),
...
send_request_receive_reqid_collection_timeout(Pid1, Pid2, Pid3) ->
    ...
    timeout = gen_statem:receive_response(
...
send_request_receive_reqid_collection_error(Pid1, Pid2, Pid3) ->
    ...
    {{error, {noproc, _}}, req2, ReqIdC4} = gen_statem:receive_response(
        ReqIdC3, 2000, true
    ),
    2 = gen_statem:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC4} = gen_statem:receive_response(
        ReqIdC4, infinity, false
    ),

    {{reply, delayed}, req1, ReqIdC4} = gen_statem:receive_response(
        ReqIdC4, infinity, false
    ),
...
send_request_wait_reqid_collection(Pid1, Pid2, Pid3) ->
    ...
    {{reply, delayed}, req2, ReqIdC4} = gen_statem:wait_response(
        ReqIdC3, infinity, true
    ),
    2 = gen_statem:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC5} = gen_statem:wait_response(
        ReqIdC4, 5678, true
    ),
    1 = gen_statem:reqids_size(ReqIdC5),

    {{reply, delayed}, req1, ReqIdC6} = gen_statem:wait_response(
        ReqIdC5, 5000, true
    ),
    0 = gen_statem:reqids_size(ReqIdC6),
...
send_request_wait_reqid_collection_timeout(Pid1, Pid2, Pid3) ->
    ...
    {{reply, delayed}, req3, ReqIdC4} = gen_statem:wait_response(
        ReqIdC4, {abs, Deadline + 1500}, false
    ),
    {{reply, delayed}, req1, ReqIdC4} = gen_statem:wait_response(
        ReqIdC4, {abs, Deadline + 1500}, false
    ),
...
send_request_wait_reqid_collection_error(Pid1, Pid2, Pid3) ->
    ...
    {{error, {noproc, _}}, req2, ReqIdC4} = gen_statem:wait_response(
        ReqIdC3, 2000, true
    ),
    2 = gen_statem:reqids_size(ReqIdC4),
...
send_request_check_reqid_collection(Pid1, Pid2, Pid3) ->
    ...
    ReqIdC1 = gen_statem:send_request(
    ...
    {{reply, delayed}, req2, ReqIdC4} = gen_statem:check_response(
        next_msg(), ReqIdC3, true
    ),
    2 = gen_statem:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC5} = gen_statem:check_response(
        next_msg(), ReqIdC4, true
    ),
    1 = gen_statem:reqids_size(ReqIdC5),

    {{reply, delayed}, req1, ReqIdC6} = gen_statem:check_response(
...
send_request_check_reqid_collection_error(Pid1, Pid2, Pid3) ->
    ...
    {{error, {noproc, _}}, req2, ReqIdC4} = gen_statem:check_response(
        next_msg(), ReqIdC3, true
    ),
    2 = gen_statem:reqids_size(ReqIdC4),

    {{reply, delayed}, req3, ReqIdC4} = gen_statem:check_response(
        next_msg(), ReqIdC4, false
    ),

    {{reply, delayed}, req1, ReqIdC4} = gen_statem:check_response(
        next_msg(), ReqIdC4, false
    ),
