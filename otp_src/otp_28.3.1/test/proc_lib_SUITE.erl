...
-export([
    ...
    sync_start_nolink/1,
    sync_start_link/1,
    sync_start_monitor/1,
    sync_start_monitor_link/1,
    sync_start_timeout/1,
    sync_start_link_timeout/1,
...
crash_1(_Config) ->
    ...
    receive
        {'EXIT', Pid0, shutdown} -> ok
    end,
    receive
        {'EXIT', Pid1, {shutdown, {a, b, c}}} -> ok
    end,
    ...
    Links4 = [
        [
            {initial_call, {?MODULE, sp1_with_label, []}},
...
sync_start_timeout(Config) when is_list(Config) ->
    ...
    receive
        {received_exit, _} = M1 -> ct:fail(M1)
    after 0 -> ok
    end,
    receive
        {received_down, _} = M2 -> ct:fail(M2)
    after 0 -> ok
    end,
    ok.
...
sp7(Tester) ->
    process_flag(trap_exit, true),
    {Pid, Mon} = proc_lib:start_monitor(
        ?MODULE, sp4, [self(), Tester], infinity, [link]
    ),
    Tester ! {sync_started, Pid},
    receive
        {'EXIT', Pid, normal} ->
...
sp8(Tester) ->
    process_flag(trap_exit, true),
    {error, timeout} = proc_lib:start(?MODULE, sp4, [self(), Tester], 500, [
        link
    ]),
    receive
    after 500 -> ok
...
sp10(Tester) ->
    process_flag(trap_exit, true),
    {{error, timeout}, Mon} = proc_lib:start_monitor(
        ?MODULE, sp4, [self(), Tester], 500, [link]
    ),
    receive
    after 500 -> ok
    end,
...
init_dont_hang(Config) when is_list(Config) ->
    ...
    try
        StartLinkRes = proc_lib:start(
            ?MODULE, init_dont_hang_init, [self()], 1000
        ),
        StartLinkRes = proc_lib:start(
            ?MODULE, init_dont_hang_init, [self()], 1000, []
        ),
...
stop(_Config) ->
    ...
    {'EXIT', {{badmatch, 2}, _Stacktrace}} =
        (catch proc_lib:stop(Pid6, crash, infinity)),
...
report_cb(_Config) ->
    ...
    ExpectedNeighbours5 =
        "; neighbours: neighbour: pid: " ++ NPidStr ++
            ", registered_name: []",
...
report_cb_chars_limit(_Config) ->
    ...
    % To separate debug calls to erlang:display(), if any.
    ct:sleep(500),
    Str1 = flatten_report_cb(Report, #{}),
    L1 = length(Str1),
