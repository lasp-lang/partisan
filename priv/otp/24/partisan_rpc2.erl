...
-define(NAME, partisan_rex).
-define(TAB_NAME, partisan_rex_nodes_observer).

-behaviour(partisan_partisan_gen_server).
...
start() ->
    partisan_gen_server:start({local,?NAME}, ?MODULE, [], ?SPAWN_OPTS).
...
start_link() ->
    ...
    %% messages. Make sure that they are stored off heap to
    %% avoid exessive GCs.
    partisan_gen_server:start_link({local,?NAME}, ?MODULE, [], ?SPAWN_OPTS).
...
stop(Rpc) ->
    partisan_gen_server:call(Rpc, stop, infinity).
...
-spec handle_call(
        term(),
        partisan_gen_server:from() | {?NAME,term()},
        state()) ->
                         {'noreply', state()} |
                         {'reply', term(), state()} |
...
handle_cast({cast, Mod, Fun, Args, Gleader}, S) ->
    _ = try
            spawn(fun() ->
                          set_group_leader(Gleader),
                          partisan_erpc:execute_cast(Mod, Fun, Args)
                  end)
        catch
            error:system_limit ->
                ok
...
reply({From, _} = To, Reply) when is_pid(From) ->
    partisan_gen_server:reply(To, Reply).


execute_call(Mod, Fun, Args) ->
    try
        {return, Return} = partisan_erpc:execute_call(Mod, Fun, Args),
        Return
    catch
        throw:Result ->
            Result;
        exit:Reason ->
            {badrpc, {'EXIT', Reason}};
        error:Reason:Stack ->
            case partisan_erpc:is_arg_error(Reason, Mod, Fun, Args) of
                true ->
                    {badrpc, {'EXIT', Reason}};
                false ->
                    RpcStack = partisan_erpc:trim_stack(Stack, Mod, Fun, Args),
...
call(N,M,F,A,T) ->
    ?RPCIFY(partisan_erpc:call(N, M, F, A, T)).
...
%% call() implementation utilizing partisan_erpc:call()...
...
do_srv_call(Node, Request, infinity) ->
    rpc_check(catch partisan_gen_server:call({?NAME,Node}, Request, infinity));
do_srv_call(Node, Request, Timeout) ->
    ...
    erlang:spawn_monitor(
      fun() ->
          ...
          %% exit signals.
          process_flag(trap_exit, true),
          Result = partisan_gen_server:call({?NAME,Node}, Request, Timeout),
...
cast(Node, Mod, Fun, Args) ->
    try
        ok = partisan_erpc:cast(Node, Mod, Fun, Args)
    catch
        error:{erpc, badarg} ->
            error(badarg)
    end,
...
multicall(Nodes, M, F, A, Timeout) ->
    %%
    %% We want to use partisan_erpc:multicall() and then convert the result
    %% instead of using partisan_erpc:send_request()/partisan_erpc:receive_response()
    %% directly. This since partisan_erpc:multicall() is able to utilize the
    %% selective receive optimization when all clauses match on the
    %% same reference. partisan_erpc:send_request()/partisan_erpc:receive_response()
    %% is not able to utilize such optimizations.
    %%
    ERpcRes = try
                  partisan_erpc:multicall(Nodes, M, F, A, Timeout)
              catch
                  error:{erpc, badarg} ->
                      error(badarg)
              end,
...
async_call(Node, Mod, Fun, Args) ->
    try
        partisan_erpc:send_request(Node, Mod, Fun, Args)
    catch
        error:{erpc, badarg} ->
            error(badarg)
    end.
