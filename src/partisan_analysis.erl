...
partisan_analysis(Tree) ->
    ...
    io:format(
        "Performing intraprocedural analysis from start of module.~n", []
    ),
    ...
    FinalResults = lists:foldl(
        fun(EntryPoint, Acc) ->
            case dict:find(EntryPoint, NamesToFunctions) of
                {ok, {_, EntryPointTree}} ->
                    Results = analysis_from_function_clause(
                        NamesToFunctions, AllSends, EntryPointTree
                    ),
                    dict:merge(
                        fun(_Key, Value1, Value2) when
    ...
    OutputDict = dict:fold(
        fun(Key, Values, Acc) when is_list(Values) ->
...
append_ann(Tag, Val, [X | Xs]) ->
    if
        tuple_size(X) >= 1, element(1, X) =:= Tag ->
...
-record(intraprocedural_state, {
    vars, out, dep, work, funs, par, sends, applys
}).
...
intraprocedural(Tree) ->
    ...
    St = intraprocedural_loop(StartFun, start, #intraprocedural_state{
        vars = Vars1,
        out = Out,
        dep = dict:new(),
        work = init_work(),
            ...
            false ->
                ...
                case dict:find(L, St1#intraprocedural_state.dep) of
                    {ok, S} ->
                        {
                            add_work(
                                set__to_list(S), St1#intraprocedural_state.work
                            ),
                            M1
                        };
                    error ->
    ...
    case type(T) of
        ...
        var ->
            ...
            case dict:find(L1, Vars) of
                ...
                error ->
                    X = empty(),
                    St1 = St#intraprocedural_state{
                        vars = dict:store(L1, X, Vars)
                    },
                    {[X], St1}
            end;
        'fun' ->
            ...
            L1 = get_label(T),
            St1 = St#intraprocedural_state{
                work = add_work([L1], St#intraprocedural_state.work),
                            ...
                            var ->
                                ...
                                case VarName of
                                    ...
                                    {_, _} ->
                                        ...
                                        St3#intraprocedural_state{
                                            applys = sets:add_element(
                                                VarName,
                                                St3#intraprocedural_state.applys
                                            )
                                        };
                                    _ ->
                                        St3
        ...
        'try' ->
            ...
            {Xs2, St2} = visit(try_body(T), L, St1#intraprocedural_state{
                vars = Vars
            }),
            Evars = bind_vars(
                try_evars(T), [X, X, X], St2#intraprocedural_state.vars
            ),
            {Xs3, St3} = visit(try_handler(T), L, St2#intraprocedural_state{
                vars = Evars
            }),
        ...
        letrec ->
            ...
            St1 = St#intraprocedural_state{
                work = add_work(Ls, St#intraprocedural_state.work),
        ...
        module ->
            ...
            visit(
                c_letrec(module_defs(T), c_tuple(module_exports(T))),
                L,
                St
            );
        map ->
...
call_site(Ls, L, Xs, St) ->
    ...
    {D, W, V} = call_site(
        ...
        L,
        Xs,
        St#intraprocedural_state.dep,
        St#intraprocedural_state.work,
        St#intraprocedural_state.vars,
...
escape([X], St) ->
    ...
    case equal_single(X0, X1) of
        ...
        false ->
            ...
            {W, Vars2} = update_esc(
                set__to_list(set__subtract(X, X0)),
                St#intraprocedural_state.work,
                Vars1,
                St#intraprocedural_state.funs
            ),
            St#intraprocedural_state{
                work = add_work([external], W), vars = Vars2
            }
    end.
    ...
    try
        case {concrete(M), concrete(F)} of
            {partisan_pluggable_peer_service_manager, forward_message} ->
                ...
                St#intraprocedural_state{
                    sends = sets:add_element(
                        MessageType, St#intraprocedural_state.sends
                    )
                };
...
generate_names_to_functions(StartFun) ->
                    ...
                    {_, _} ->
                        ...
                        case length(Anns) >= 3 of
                            true ->
                                ...
                                {N2Fs, CandidateTree, CandidateLabel,
                                    CandidateAnns};
                            false ->
                                %% Match!
                                case
                                    CandidateLabel =/= undefined andalso
                                        CandidateAnns =/= undefined
                                of
                                    false ->
    ...
    {NameToFuns, _, _, _} = reverse_postorder_fold(
        FoldFun,
        {NameToFuns0, CandidateTree0, CandidateLabel0, CandidateAnns0},
        StartFun
    ),
...
analysis_from_function_clause(NamesToFunctions, Top, Tree) ->
                ...
                case Found of
                    false ->
                        ...
                        Dict = lists:foldl(
                            fun(Clause, Dict1) ->
                                %% Get message type.
                                MessageType = message_type_from_function_clause(
                                    Clause
                                ),
    ...
    {_, Results} = reverse_postorder_fold(
        FindFunctionClauseFun, {false, dict:new()}, Tree
    ),
...
interprocedural_loop(NamesToFunctions, Top, [Tree | Rest], Sends0) ->
    ...
    WorklistAdditions = lists:flatmap(
        fun(X) ->
    ...
    Sends =
        case lists:member(top, sets:to_list(Sends1)) of
            false ->
    ...
    interprocedural_loop(
        NamesToFunctions, Top, Rest ++ WorklistAdditions, Sends
    ).
