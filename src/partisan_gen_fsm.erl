...
send_event(Name, Event) ->
	...
	logger:info("[cmeik] sending event to node: ~p name: ~p event: ~p", [Node, Process, Event]),
	partisan_pluggable_peer_service_manager:forward_message(Node, undefined, Process, {'$gen_event', Event}, []),
	ok.

	% logger:info("[cmeik] sending event to name: ~p with event: ~p", [Name, Event]),
...
sync_send_event(Name, Event) ->
	...
	{ok,Res} ->
	    Res;
	{'EXIT',Reason} ->
		logger:info("[cmeik] exit for ~p", [Reason]),
...
send_all_state_event(Name, Event) ->
	...
	logger:info("[cmeik] sending all state event from node: ~p to node ~p name ~p event ~p", [node(), Node, Process, Event]),
	partisan_pluggable_peer_service_manager:forward_message(Node, undefined, Process, {'$gen_all_state_event', Event}, []),
	ok.

	% logger:info("[cmeik] sending all state event to name: ~p and event: ~p", [Name, Event]),
    % Name ! {'$gen_all_state_event', Event},
    % ok.

sync_send_all_state_event(Name, Event) ->
	logger:info("calling sync_send_all_event with name ~p", [Name]),
...
sync_send_all_state_event(Name, Event, Timeout) ->
	...
	{ok,Res} ->
	    Res;
	{'EXIT',Reason} ->
		logger:info("[cmeik] exit for ~p after node: ~p sending event: ~p to ~p", [Reason, node(), Event, Name]),
