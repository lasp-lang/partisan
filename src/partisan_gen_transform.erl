    ...
    case get_abstract_code(OrigModule) of
        {ok, Forms} ->
            ...
            case
                compile:forms(
                    PatchedForms, [
                        binary, return_errors, deterministic, debug_info
                    ]
                )
            of
...
%% straight to BEAM via `generate_all/1' (see `generate/1'); pretty-printing
%% back to source with `erl_pp' mis-renders auto-imported BIFs that collide with
%% locally-defined functions (e.g. `spawn_opt' → unbounded recursion in
%% `partisan_proc_lib'), so a source-file path is unsafe and was a trap for
%% anyone who switched to it.
