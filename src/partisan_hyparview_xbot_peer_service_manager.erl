...
handle_call({inject_partition, Origin, TTL}, _From,
    ...
    logger:debug("Injecting partition; Origin: ~p Myself: ~p TTL: ~p",
               [Origin, Myself, TTL]),
...
handle_call(members, _From, #state{myself=Myself,
                                   active=Active}=State) ->
    logger:debug("Node ~p active view: ~p", [Myself, members(Active)]),
...
handle_call(connections, _From,
    ...
    Cs = lists:map(fun(Peer) ->
                    ...
                    logger:debug("peer ~p connection pids: ~p",
                               [Peer, MappedPids]),
                    {Peer, MappedPids}
...
handle_cast({join, Peer},
    ...
    logger:debug("Node ~p sends the JOIN message to ~p", [Myself0, Peer]),
...
handle_info(random_promotion, #state{myself=Myself,
    ...
    State = case has_reached_the_limit({active, Active0, Reserved0},
                ...
                true ->
                    ...
                    State0;
                false ->
                    % logger:info("Random promotion for node ~p", [Myself0]),
...
handle_info({'EXIT', From, Reason},
            #state{myself=Myself,
                   ...
                   passive=Passive0,
                   connections=Connections0}=State0) ->
    logger:debug("connection pid ~p died due to ~p",
               [From, Reason]),
...
process_candidate([OldNode | RestActiveNodes], CandidateNode, #state{myself=InitiatorNode, connections=Connections}=InitiatorState) ->
	...
	if IsBetter ->
		...
		logger:debug("XBOT: Optimization message sent to Node ~p from ~p", [CandidateName, InitiatorName]);
	   true ->
...
handle_message({join, Peer, PeerTag, PeerEpoch},
    ...
    logger:debug("Node ~p received the JOIN message from ~p, epoch ~p",
    ...
    State = case IsAddable andalso NotInActiveView of
        true ->
            ...
            case partisan_peer_service_connections:find(Peer, Connections1) of
                {ok, _Entries} ->
                    logger:debug("Adding peer node ~p to the active view",
                                [Peer]),
                    %% Add to active view.
                    ...
                    Connections = lists:foldl(
                                    fun(P, AccConnections0) ->
                                            ...
                                            logger:debug("Forwarding join of ~p to active view peer ~p",
                                                        [Peer, P]),
                                            do_send_message(
                                              P,
                    ...
                    logger:debug("Node ~p active view: ~p", [Myself0, members(State1#state.active)]),
        ...
        false ->
            logger:debug("Peer node ~p will not be added to the active view",
                       [Peer]),
            State0
    end,
...
handle_message({neighbor, Peer, PeerTag, DisconnectId, _Sender},
    ...
    logger:debug("Node ~p received the NEIGHBOR message from ~p with ~p",
...
handle_message({forward_join, Peer, PeerTag, PeerEpoch, TTL, Sender},
               #state{myself=Myself0,
                      ...
                      sent_message_map=SentMessageMap0,
                      recv_message_map=RecvMessageMap0}=State0) ->
    logger:debug("Node ~p received the FORWARD_JOIN message from ~p about ~p",
               [Myself0, Sender, Peer]),

    ActiveViewSize = sets:size(Active0),
    State = case TTL =:= 0 orelse ActiveViewSize =:= 1 of
        true ->
            logger:debug("FORWARD_JOIN: ttl(~p) expired or only one peer in active view (~p), "
        ...
        false ->
            ...
            State2 = case TTL =:= prwl() of
                         true ->
                             logger:debug("FORWARD_JOIN: Passive walk ttl expired, adding ~p to "
            ...
            case select_random(Active0, [Sender, Myself0, Peer]) of
                    ...
                    case IsAddable1 andalso NotInActiveView1 of
                        true ->
                            ...
                            case partisan_peer_service_connections:find(Peer, Connections3) of
                                {ok, _Entries} ->
                                    logger:debug("FORWARD_JOIN: No node for forward, adding ~p to active view",
                                                [Peer]),
                                    %% Add to our active view.
                ...
                Random ->
                    ...
                    logger:debug("FORWARD_JOIN: forwarding to ~p",
                               [Random]),
                    %% Forward join.
                    do_send_message(
                        Random,
...
handle_message({disconnect, Peer, DisconnectId},
               #state{myself=Myself0,
                      ...
                      connections=Connections0,
                      recv_message_map=RecvMessageMap0}=State0) ->
    logger:debug("Node ~p received the DISCONNECT message from ~p with ~p",
    ...
    case is_valid_disconnect(Peer, DisconnectId, RecvMessageMap0) of
        ...
        true ->
            %% Remove from active
            Active = sets:del_element(Peer, Active0),
            logger:debug("Node ~p active view: ~p", [Myself0, members(Active)]),
            ...
            State = case sets:size(Active) == 1 of
                        true ->
                            ...
                            logger:debug("Node ~p is isolated, moving random peer ~p from passive "
                                       "to active view",
                                       [RandomPeer, Myself0]),
                            move_peer_from_passive_to_active(RandomPeer,
...
handle_message({neighbor_request, Peer, Priority, PeerTag, DisconnectId, Exchange},
               #state{myself=Myself0,
                      ...
                      sent_message_map=SentMessageMap0,
                      recv_message_map=RecvMessageMap0}=State0) ->
    logger:debug("Node ~p received the NEIGHBOR_REQUEST message from ~p with ~p",
               [Myself0, Peer, DisconnectId]),
        ...
        case neighbor_acceptable(Priority, PeerTag, State0) of
            true ->
                case is_addable(DisconnectId, Peer, SentMessageMap0) of
                    true ->
                        logger:debug("Node ~p accepted neighbor peer ~p",
                                   [Myself0, Peer]),
                        LastDisconnectId = get_current_id(Peer, RecvMessageMap0),
                        ...
                        State1;
                    false ->
                        logger:debug("Node ~p rejected neighbor peer ~p",
                                   [Myself0, Peer]),
...
handle_message({neighbor_rejected, Peer, Exchange},
               #state{myself=Myself0,
                      connections=Connections0} = State0) ->
    logger:debug("Node ~p received the NEIGHBOR_REJECTED message from ~p",
               [Myself0, Peer]),
...
handle_message({neighbor_accepted, Peer, PeerTag, DisconnectId, Exchange},
               #state{myself=Myself0,
                      sent_message_map=SentMessageMap0} = State0) ->
    logger:debug("Node ~p received the NEIGHBOR_ACCEPTED message from ~p with ~p",
               [Myself0, Peer, DisconnectId]),
...
handle_message({shuffle, Exchange, TTL, Sender},
               #state{myself=Myself,
                      ...
                      passive=Passive0,
                      connections=Connections0}=State0) ->
    logger:debug("Node ~p received the SHUFFLE message from ~p",
               [Myself, Sender]),
...
handle_message({relay_message, Node, Message}, #state{out_links=OutLinks, connections=Connections}=State) ->
    logger:debug("Node ~p received tree relay to ~p", [partisan_peer_service_manager:mynode(), Node]),
...
handle_message({optimization_reply, true, _, InitiatorNode, CandidateNode, undefined}, State) ->%#state{active=Active}=State) ->
	#{name := InitiatorName} = InitiatorNode,
	#{name := CandidateName} = CandidateNode,
	logger:debug("XBOT: Received optimization reply message at Node ~p from ~p", [InitiatorName, CandidateName]),
	...
	logger:debug("XBOT: Finished optimization round started by Node ~p ", [InitiatorName]),
	{noreply, State};
...
handle_message({optimization, _, OldNode, InitiatorNode, CandidateNode, undefined},
	...
	#{name := CandidateName} = CandidateNode,
	#{name := InitiatorName} = InitiatorNode,
	logger:debug("XBOT: Received optimization message at Node ~p from ~p", [CandidateName, InitiatorName]),
	...
	if not Check ->
			...
			logger:debug("XBOT: Sending optimization reply message to Node ~p from ~p", [InitiatorName, CandidateName]);
		true ->
			...
			logger:debug("XBOT: Sending replace message to Node ~p from ~p", [DisconnectName, CandidateName])
	end,
	{noreply, State};
...
handle_message({replace_reply, true, OldNode, InitiatorNode, CandidateNode, DisconnectNode},
	...
	#{name := DisconnectName} = DisconnectNode,
	#{name := CandidateName} = CandidateNode,
	logger:debug("XBOT: Received replace reply message at Node ~p from ~p", [CandidateName, DisconnectName]),
	...
	logger:debug("XBOT: Sending optimization reply to Node ~p from ~p", [InitiatorName, CandidateName]),
	{noreply, State};
...
handle_message({replace, _, OldNode, InitiatorNode, CandidateNode, DisconnectNode}, #state{connections=Connections}=State) ->
	...
	logger:debug("XBOT: Received replace message at Node ~p from ~p", [DisconnectName, CandidateName]),
	...
	if not Check ->
			...
			logger:debug("XBOT: Sending replace reply to Node ~p from ~p", [CandidateName, DisconnectName]);
		true ->
			...
			logger:debug("XBOT: Sending switch to Node ~p from ~p", [OldName, DisconnectName])
	end,
	{noreply, State};
...
handle_message({switch_reply, true, OldNode, InitiatorNode, CandidateNode, DisconnectNode},
	...
	#{name := CandidateName} = CandidateNode,
	#{name := OldName} = OldNode,
	logger:debug("XBOT: Received switch reply message at Node ~p from ~p", [DisconnectName, OldName]),
...
handle_message({switch, _, OldNode, InitiatorNode, CandidateNode, DisconnectNode},
	...
	#{name := DisconnectName} = DisconnectNode,
	#{name := OldName} = OldNode,
	logger:debug("XBOT: Received switch message at Node ~p from ~p", [OldName, DisconnectName]),
	...
	if Check ->
			...
			logger:debug("XBOT: Sending switch reply to Node ~p from ~p", [DisconnectName, OldName]);
		true ->
			NodeConnections = partisan_util:maybe_connect(DisconnectNode, Connections),
...
is_better_node_by_latency({NewTime, pong}, {OldTime, pong}) ->
	logger:debug("XBOT: Checking is better - OldTime ~p - NewTime ~p", [OldTime, NewTime]),
    ...
    case data_root() of
        ...
        Dir ->
            ...
            case file:delete(File) of
                ok ->
                    logger:debug("Leaving cluster, removed cluster_state");
                {error, Reason} ->
                    logger:debug("Unable to remove cluster_state for reason ~p", [Reason])
            end
    end.
    ...
    Connections =
        case partisan_peer_service_connections:find(Node, Connections0) of
            ...
            {ok, [{_ListenAddr, _Channel, Pid}|_]} ->
                %% Stop;
                logger:debug("disconnecting node ~p by stopping connection pid ~p",
                           [Node, Pid]),
                gen_server:stop(Pid),
...
do_send_message(Node, Message, Connections, Options) ->
    ...
    case partisan_peer_service_connections:find(Node, Connections) of
        {ok, []} ->
            logger:debug("Node disconnected when sending ~p to ~p!", [Message, Node]),
            logger:debug("Broadcast mode: ~p, transitive: ~p", [partisan_config:get(broadcast, false), proplists:get_value(transitive, Options, false)]),
        ...
        {ok, Entries} ->
            try
                ...
                Reason:Error ->
                    logger:debug("failed to send a message to ~p due to ~p:~p", [Node, Reason, Error]),
                    {error, Error}
            end;
        {error, not_found} ->
            logger:debug("Node not yet connected when sending ~p to ~p!", [Message, Node]),
            logger:debug("Broadcast mode: ~p, transitive: ~p", [partisan_config:get(broadcast, false), proplists:get_value(transitive, Options, false)]),
...
add_to_active_view(#{name := Name}=Peer, Tag,
    ...
    case IsNotMyself andalso NotInActiveView of
        true ->
            ...
            logger:debug("Node ~p adds ~p to active view with tag ~p",
                       [Myself, Peer, Tag]),
            ...
            Reserved = case dict:find(Tag, Reserved0) of
                {ok, undefined} ->
                    logger:debug("Node added to reserved slot!"),
                    dict:store(Tag, Peer, Reserved0);
                {ok, _} ->
                    %% Slot already filled, treat this as a normal peer.
                    logger:debug("Node added to active view, but reserved slot already full!"),
                    Reserved0;
                error ->
                    logger:debug("Tag is not reserved: ~p ~p", [Tag, Reserved0]),
                    Reserved0
            end,
...
add_to_passive_view(#{name := Name}=Peer,
    ...
    % logger:debug("Adding ~p to passive view on ~p", [Peer, Myself]),
...
drop_random_element_from_active_view(
    ...
    case select_random(Active0, [Myself0, ReservedPeers]) of
        undefined ->
            State0;
        Peer ->
            logger:debug("Removing and disconnecting peer: ~p", [Peer]),
...
move_peer_from_passive_to_active(Peer,
        #state{myself=Myself0,
               ...
               connections=Connections0,
               recv_message_map=RecvMessageMap0}=State0) ->
    logger:debug("Node ~p sends the NEIGHBOR_REQUEST to ~p", [Myself0, Peer]),
...
propagate_partition_injection(Ref, Origin, TTL, Peer, Connections) ->
    logger:debug("Forwarding partition request to: ~p", [Peer]),
...
%% @private
do_tree_forward(Node, Message, Connections, Options) ->
    logger:debug("Attempting to forward message from ~p to ~p.", [partisan_peer_service_manager:mynode(), Node]),
    ...
    OutLinks = case UserOutLinks of
        undefined ->
            try retrieve_outlinks() of
                ...
                _:_ ->
                    logger:error("Outlinks retrieval failed!"),
                    []
            end;
        OL ->
            OL
    ...
    lists:foreach(fun(N) ->
        logger:debug("Forwarding relay message to node ~p for node ~p from node ~p", [N, Node, partisan_peer_service_manager:mynode()]),
