...
handle_info({Tag, _RawSocket, Data}, State=#state{socket=Socket}) when ?DATA_MSG(Tag) ->
    case partisan_config:get(tracing, ?TRACING) of
        true ->
            logger:info("Recevied data from socket: ~p", [decode(Data)]);
        false ->
            ok
    end,
...
handle_info({Tag, _RawSocket, Reason}, State=#state{socket=Socket}) when ?ERROR_MSG(Tag) ->
    logger:error("Connection socket ~p errored out, closing", [Socket]),
    {stop, Reason, State};
handle_info({Tag, _RawSocket}, State=#state{socket=Socket}) when ?CLOSED_MSG(Tag) ->
    case partisan_config:get(tracing, ?TRACING) of
        true ->
            logger:info("Connection socket ~p has been remotely closed", [Socket]);
        false ->
            ok
    end,
...
handle_message({hello, Node},
    ...
    case maybe_connect_disterl(Node) of
        ok ->
            ...
            % logger:info("got hello reply from ~p, sending local state", [Node]),
            send_message(Socket, {state, Tag, LocalState}),
            ok;
        error ->
            logger:info("Node could not be connected."),
...
handle_message(Message, _State) ->
    ...
    case partisan_config:get(tracing, ?TRACING) of
        true ->
            logger:info("Received message from peer ~p: ~p", [Peer, Message]);
        false ->
            ok
    end,
    ...
    case partisan_config:get(tracing, ?TRACING) of
        true ->
            logger:info("Dispatched ~p to manager.", [Message]);
        false ->
            ok
    end,
