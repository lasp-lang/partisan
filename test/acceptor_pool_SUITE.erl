...
init_per_testcase(start_error, Config) ->
    Config;
init_per_testcase(Change, Config) when
    Change == change_spec; Change == change_ignore; Change == change_error
->
    Config;
...
shutdown_children(Config) ->
    ...
    Reason =
        case ?config(shutdown, Config) of
            brutal_kill -> killed;
            _ -> shutdown
        end,

    receive
        {'DOWN', Ref1, _, _, Reason} -> ok
    end,
    receive
        {'DOWN', Ref2, _, _, Reason} -> ok
    end,
    receive
        {'DOWN', ARef, _, _, Reason} -> ok
    end,
...
kill_children(Config) ->
    ...
    receive
        {'DOWN', ARef, _, _, killed} -> ok
    end,
    receive
        {'DOWN', Ref1, _, _, killed} -> ok
    end,
    receive
        {'DOWN', Ref2, _, _, killed} -> ok
    end,
