...
%% How many milliseconds to delay outgoing messages?
-define(EGRESS_DELAY, 0).
%% How many millisconds to delay incoming messages?
-define(INGRESS_DELAY, 0).
%% Should communication be partitioned by vnode identifier?
...
annotations_test(_Config) ->
    ?LOG_INFO("~p: starting nodes!", [?MODULE]),
    ...
    {ok, Base} = file:get_cwd(),
    CounterexampleFilePath = filename:join([
        Base, "../../", ?COUNTEREXAMPLE_FILE
    ]),
    case file:consult(CounterexampleFilePath) of
        {ok, [Counterexample]} ->
            ...
            AnnotationsFile =
                BasePath ++ "/annotations/partisan-annotations-" ++
                    ModuleString,

            case filelib:is_file(AnnotationsFile) of
                false ->
                    debug("Annotations file doesn't exist: ~p~n", [
                        AnnotationsFile
                    ]),
            ...
            {ok, RawCausalityAnnotations} = dict:find(
                causality, AllAnnotations
            ),
            debug("Raw causality annotations loaded: ~p~n", [
                RawCausalityAnnotations
            ]),

            CausalityAnnotations = dict:from_list(RawCausalityAnnotations),
            debug("Causality annotations loaded: ~p~n", [
                dict:to_list(CausalityAnnotations)
            ]),
            ...
            lists:foreach(
                fun(_) ->
                    ...
                    debug("MessageTypesFromTraceLines: ~p", [
                        MessageTypesFromTraceLines
                    ]),

                    %% Verify annotations.
                    verify_annotations(
                        CausalityAnnotations, MessageTypesFromTraceLines
                    )
                end,
                lists:seq(1, 1)
            ),
...
model_checker_test(_Config) ->
    ...
    %% Compile cover.
    {ok, Base} = file:get_cwd(),
    CoverModule = filename:join([
        Base,
        "../../../../protocols/" ++ atom_to_list(ImplementationModule) ++ ".erl"
    ]),
    case filelib:is_file(CoverModule) of
        true ->
            {ok, _ImplementationModule} = cover:compile(
                filename:join([
                    Base,
                    "../../../../protocols/" ++
                        atom_to_list(ImplementationModule) ++ ".erl"
                ])
            ),
            ok;
        false ->
    ...
    case file:consult(CounterexampleFilePath) of
        {ok, [Counterexample]} ->
            ...
            init(
                ...
                TraceFile,
                ReplayTraceFile,
                CounterexampleConsultFile,
                RebarCounterexampleConsultFile,
                PreloadOmissionFile
            ),
            ...
            ImplementationModule = implementation_module(),
            {ok, AnalysisFileResults} = cover:analyze_to_file(
                ImplementationModule, [html]
            ),
            CoverageFile = os:cmd(
                "find " ++ BasePath ++ " -name " ++ AnalysisFileResults ++
                    " | tail -1"
            ),
            debug("Coverage file: ~p", [
                lists:sublist(CoverageFile, 1, length(CoverageFile) - 1)
            ]),
...
execute(Nodes, PerformPreloads, Replaying, Shrinking, Tracing, {M, F, A}) ->
    ...
    TraceIdentifier =
        atom_to_list(system_model()) ++ "_" ++
            integer_to_list(TraceRandomNumber),
...
init(
    ...
    %% Open the annotations file.
    AnnotationsFile =
        BasePath ++ "/annotations/partisan-annotations-" ++ ModuleString,
    ...
    PreconditionsPresent = ensure_preconditions_present(
        Causality, CausalityAnnotations, BackgroundAnnotations
    ),
    ...
    {ok, [{TestModule, TestFunction, [TestCommands]}]} = file:consult(
        CounterexampleConsultFile
    ),
    ...
    [
        debug(" ~p.~n", [TestFinalCommand])
     || TestFinalCommand <- TestFinalCommands
    ],

    %% Write the schedule out.
    {ok, CounterexampleIo} = file:open(RebarCounterexampleConsultFile, [
        write, {encoding, utf8}
    ]),
    io:format(CounterexampleIo, "~p.~n", [
        {TestModule, TestFunction, [TestFinalCommands]}
    ]),
    ...
    TraceLinesWithoutFailure = lists:filter(
        fun(Command) ->
            case Command of
                {_, {_, [forced_failure]}} ->
    ...
    analyze(
        ...
        TraceFile,
        Causality,
        CausalityAnnotations,
        BackgroundAnnotations,
        PreviousIteration,
        PreviousClassificationsExplored,
        [{TraceLinesWithoutFailure, [], [], []}]
    ),
...
filter_trace_lines(_, TraceLines, BackgroundAnnotations) ->
    ...
    MessageTraceLines = lists:filter(
        fun({Type, Message}) ->
            case Type =:= pre_interposition_fun of
                true ->
                    {_TracingNode, InterpositionType, _OriginNode,
...
analyze(
    ...
    _NumFailed0,
    _NumPruned0,
    _PreloadOmissionFile,
    _ReplayTraceFile,
    _TraceFile,
    _Causality,
    _CausalityAnnotations,
    _BackgroundAnnotations,
    _PreviousIteration,
    _PreviousClassificationsExplored,
    []
) ->
    ok;
analyze(
    ...
    PreviousClassificationsExplored,
    [
        {TraceLines, BaseOmissions, DifferenceTypes, DifferenceTraceLines}
        | RestTraceLines
    ]
) ->
    debug(
        "Beginning analyze pass: ~p with previous iteration: ~p, traces remaining: ~p~n",
        [Pass, PreviousIteration, length(RestTraceLines)]
    ),

    %% Generate the powerset of tracelines.
    {MessageTraceLines, MessageTraceLinesPowerset} =
        case length(DifferenceTraceLines) > 0 of
            true ->
                ...
                debug("Using difference types for powerset generation: ~p~n", [
                    DifferenceTypes
                ]),

                %% Use message trace.
                FilteredTraceLines = filter_trace_lines(
                    implementation_module(), TraceLines, BackgroundAnnotations
                ),

                %% Generate powerset using *only* the new messages.
                FilteredDifferenceTraceLines = filter_trace_lines(
                    implementation_module(),
                    DifferenceTraceLines,
                    BackgroundAnnotations
                ),
                {Time, L} = timer:tc(fun() ->
                    powerset(FilteredDifferenceTraceLines)
                end),

                % debug("FilteredDifferenceTraceLines: ~p~n", [FilteredDifferenceTraceLines]),
                % debug("DistanceTraceLines: ~p~n", [DifferenceTraceLines]),
                debug("Number of messages in DifferenceTraceLines: ~p~n", [
                    length(DifferenceTraceLines)
                ]),
                ...
                PrefixedPowersetTraceLines = lists:map(
                    fun(X) -> BaseOmissions ++ X end, L
                ),

                %% TODO: Remove me.
                FinalPowerset =
                    [lists:nth(1, PrefixedPowersetTraceLines)] ++
                        [
                            lists:nth(
                                length(PrefixedPowersetTraceLines),
                                PrefixedPowersetTraceLines
                            )
                        ],
            ...
            false ->
                ...
                debug(
                    "Beginning powerset generation, length(FilteredTraceLines): ~p~n",
                    [length(FilteredTraceLines)]
                ),
                {Time, FinalPowerset} = timer:tc(fun() ->
                    powerset(FilteredTraceLines)
                end),
                debug("Number of message sets in powerset: ~p~n", [
                    length(FinalPowerset)
                ]),
                debug("Powerset generation took: ~p~n", [Time]),
                {FilteredTraceLines, FinalPowerset}
        end,
    ...
    %% Generate schedules.
    {GenIteration, GenNumPassed, GenNumFailed, GenNumPruned,
        GenClassificationsExplored, GenAdditionalTraces} = lists:foldl(
        fun(
            Omissions,
            {GenIteration0, GenNumPassed0, GenNumFailed0, GetNumPruned0,
                GenClassificationsExplored0, GenAdditionalTraces0}
        ) ->
            % debug("~n", []),
            ...
            EarlyCausality = schedule_valid_causality(
                Causality, CandidateTrace
            ),

            EarlyClassification0 = classify_schedule(
                3, CausalityAnnotations, CandidateTrace
            ),
            ...
            EarlyOmissions =
                case
                    sets:is_subset(BaseOmissionsSet, OmissionsSet) orelse
                        BaseOmissionsSet =:= OmissionsSet
                of
                    true ->
                        true;
                    false ->
            ...
            case
                EarlyOmissions andalso EarlyCausality andalso
                    not classification_explored(
                        EarlyClassification, GenClassificationsExplored0
                    )
            of
                true ->
                    debug("~n", []),
                    ...
                    debug(
                        "Generating new trace based on message omissions (~p omissions, iteration ~p): ~n",
                        [length(Omissions), GenIteration0]
                    ),

                    {FinalTraceLines, _, _, PrefixMessageTypes,
                        OmittedMessageTypes, ConditionalMessageTypes, _,
                        _BackgroundOmissions} = lists:foldl(
                        fun(
                            {Type, Message} = Line,
                            {FinalTrace0, FaultsStarted0, AdditionalOmissions0,
                                PrefixMessageTypes0, OmittedMessageTypes0,
                                ConditionalMessageTypes0, FaultedNodes0,
                                BackgroundOmissions0}
                        ) ->
                            case Type =:= pre_interposition_fun of
                                true ->
                                    {TracingNode, InterpositionType, OriginNode,
                                                ...
                                                forward_message ->
                                                    ...
                                                    of
                                                        true ->
                                                            ...
                                                            FaultedNodes = update_faulted_nodes(
                                                                TraceLines,
                                                                {Type, Message},
                                                                Omissions,
                                                                BackgroundAnnotations,
                                                            ...
                                                            {FinalTrace0,
                                                                FaultsStarted0,
                                                                AdditionalOmissions0 ++
                                                                    [
                                                                        ReceiveOmission
                                                                    ],
                                                                PrefixMessageTypes0,
                                                                OmittedMessageTypes0 ++
                                                                    [
                                                                        message_type(
                                                                            Message
                                                                        )
                                                                    ],
                                                                ConditionalMessageTypes0,
                                                                FaultedNodes,
                                                                BackgroundOmissions0};
                                                        false ->
                    ...
                    ScheduleValidCausality = schedule_valid_causality(
                        Causality,
                        PrefixMessageTypes,
                        OmittedMessageTypes,
                        ConditionalMessageTypes
                    ),
                    ...
                    ScheduleValid =
                        ScheduleValidCausality andalso ScheduleValidOmissions,
                    ...
                    ClassifySchedule = classify_schedule(
                        3,
                        CausalityAnnotations,
                        PrefixMessageTypes,
                        OmittedMessageTypes,
                    ...
                    case os:getenv("PRELOAD_SCHEDULES") of
                        "true" ->
                            debug(
                                "Inserting into generated schedule list.~n", []
                            ),
                            ...
                            true = ets:insert(
                                ?SCHEDULES,
                                {GenIteration0,
                                    {Omissions, FinalTraceLines,
                                        ClassifySchedule, ScheduleValid}}
                            ),

                            {
                                ...
                                GenNumFailed0,
                                GetNumPruned0,
                                GenClassificationsExplored0,
                                GenAdditionalTraces0
                            };
                        _Other ->
        ...
        {PreviousIteration, NumPassed0, NumFailed0, NumPruned0,
            PreviousClassificationsExplored, RestTraceLines},
        TracesToIterate
    ),

    %% Run generated schedules stored in the ETS table.
    {PreloadNumPassed, PreloadNumFailed, PreloadNumPruned,
        PreloadClassificationsExplored, PreloadAdditionalTraces} = ets:foldl(
        fun(
            {Iteration,
                {Omissions, FinalTraceLines, ClassifySchedule, ScheduleValid}},
            {PreloadNumPassed0, PreloadNumFailed0, PreloadNumPruned0,
                PreloadClassificationsExplored0, PreloadAdditionalTraces0}
        ) ->
            ...
            case
                execute_schedule(
                    ...
                    TraceLines,
                    {Iteration,
                        {Omissions, FinalTraceLines, ClassifySchedule,
                            ScheduleValid}},
                    PreloadClassificationsExplored0,
                    PreloadAdditionalTraces0
                )
            of
                pruned ->
                    {PreloadNumPassed0, PreloadNumFailed0,
                        PreloadNumPruned0 + 1, PreloadClassificationsExplored0,
                        PreloadAdditionalTraces0};
                {passed, ClassificationsExplored, NewTraces} ->
                    {
                        PreloadNumPassed0 + 1,
                ...
                {failed, ClassificationsExplored, NewTraces} ->
                    {PreloadNumPassed0, PreloadNumFailed0 + 1,
                        PreloadNumPruned0, ClassificationsExplored, NewTraces};
                invalid ->
                    {PreloadNumPassed0, PreloadNumFailed0,
                        PreloadNumPruned0 + 1, PreloadClassificationsExplored0,
                        PreloadAdditionalTraces0}
        ...
        {GenNumPassed, GenNumFailed, GenNumPruned, GenClassificationsExplored,
            GenAdditionalTraces},
        ?SCHEDULES
    ),
    ...
    debug("~n", []),
    debug(
        "Pass: ~p, Iterations: ~p - ~p, Aggregate: Passed: ~p, Failed: ~p, Pruned: ~p~n",
        [
            Pass,
            PreviousIteration,
            GenIteration,
    ...
    case os:getenv("RECURSIVE") of
        ...
        _ ->
            analyze(
                ...
                BackgroundAnnotations,
                GenIteration,
                PreloadClassificationsExplored,
                RestTraceLines
            )
    end.
...
is_subtrace(Trace1, Trace2) ->
    ...
    FilteredTrace2 = lists:filter(
        fun(T2) -> lists:member(T2, Trace1) end, Trace2
    ),
...
%% @private
schedule_valid_causality(
    Causality, PrefixSchedule, _OmittedSchedule, ConditionalSchedule
) ->
    ...
    RequirementsMet = lists:foldl(
        fun(Type, Acc) ->
            % debug("=> Type: ~p~n", [Type]),

            {AtLeastOneDependency, SatifyingDependency} = lists:foldl(
                fun({K, V}, {_AtLeastOne, Found}) ->
                    % debug("=> V: ~p~n", [V]),
...
identify_minimal_witnesses() ->
    ...
    Witnesses = ets:foldl(
        fun(
            {_, {_Iteration, FinalTraceLines, _Omissions, Status} = Candidate},
            Witnesses1
        ) ->
            ...
            case Status of
                true ->
                    ...
                    AllSupertracesPass = ets:foldl(
                        fun(
                            {_,
                                {_Iteration1, FinalTraceLines1, _Omissions1,
                                    Status1}},
                            AllSupertracesPass1
                        ) ->
                            case
                                is_supertrace(FinalTraceLines1, FinalTraceLines)
                            of
                                true ->
    ...
    %% Identify minimal.
    MinimalWitnesses = lists:foldl(
        fun(
            {Iteration, FinalTraceLines, _Omissions, _Status} = Witness,
            MinimalWitnesses
        ) ->
            % debug("=> looking at iteration ~p~n", [Iteration]),

            %% See if any of the traces are subtraces of this.
            StillMinimal = lists:foldl(
                fun(
                    {Iteration1, FinalTraceLines1, _Omissions1, _Status1},
                    StillMinimal1
                ) ->
                    %% Is this other trace a subtrace of me?  If so, discard us
                    case
                        is_subtrace(FinalTraceLines1, FinalTraceLines) andalso
...
classify_schedule(
    ...
    CausalityAnnotations = dict:fold(
        fun(Key, Value, Acc) ->
            case Value of
                [true] ->
                    Acc;
    ...
    Classification = lists:foldl(
        fun({Type, Preconditions}, Dict0) ->
            Result = lists:foldl(
                fun(Precondition, Acc) ->
                    case Precondition of
                        {PreconditionType, N} ->
                            Num = length(
                                lists:filter(
                                    fun(T) -> T =:= PreconditionType end,
                                    DerivedSchedule
                                )
                            ),
                        ...
                        {{_, PreconditionType}, N} ->
                            Num = length(
                                lists:filter(
                                    fun(T) ->
                                        T =:=
                                            {forward_message, PreconditionType}
                                    end,
                                    CandidateTrace
                                )
                            ),
...
%% @private
ensure_preconditions_present(
    Causality, CausalityAnnotations, BackgroundAnnotations
) ->
    %% Get all messages that are the result of other messages.
    CausalMessages = lists:foldl(
        fun({_, Messages}, Acc) ->
            Messages ++ Acc
        end,
        [],
    ...
    lists:foldl(
        fun(Message, Acc) ->
            case
                lists:keymember(Message, 1, dict:to_list(CausalityAnnotations))
            of
                true ->
                    Acc andalso true;
                false ->
                    case
                        lists:member(element(2, Message), BackgroundAnnotations)
                    of
                        true ->
                            Acc andalso true;
                        false ->
...
execute_schedule(
    ...
    TraceFile,
    TraceLines,
    {Iteration, {Omissions, FinalTraceLines, ClassifySchedule, ScheduleValid}},
    ClassificationsExplored0,
    NewTraces0
) ->
    ...
    case ScheduleValid of
        ...
        true ->
            case
                classification_explored(
                    Classification, ClassificationsExplored0
                ) andalso pruning()
            of
                true ->
                    debug("Classification: ~p~n", [Classification]),
                    debug("Classifications explored: ~p~n", [
                        ClassificationsExplored0
                    ]),
                ...
                false ->
                    ...
                    ok = partisan_trace_file:write(
                        PreloadOmissionFile, Omissions
                    ),

                    %% Write out replay trace.
                    debug("Writing out new replay trace file!~n", []),
                    ok = partisan_trace_file:write(
                        ReplayTraceFile, FinalTraceLines
                    ),

                    ClassificationsExplored =
                        ClassificationsExplored0 ++ [Classification],
                    ...
                    debug("=> OmissionTypes for this test: ~p~n", [
                        OmissionTypes
                    ]),
                    ...
                    debug(
                        "=> Executed test and test contained ~p lines compared to original trace with ~p lines.~n",
                        [length(NewTraceLines), length(TraceLines)]
                    ),

                    MessageTypesFromTraceLines = message_types(TraceLines),
                    % debug("=> MessageTypesFromTraceLines: ~p~n", [MessageTypesFromTraceLines]),
                    MessageTypesFromNewTraceLines = message_types(
                        NewTraceLines
                    ),
                    ...
                    DifferenceTypes =
                        lists:usort(MessageTypesFromNewTraceLines) --
                            lists:usort(MessageTypesFromTraceLines),
                    debug("=> DifferenceTypes: ~p~n", [DifferenceTypes]),

                    NewTraces =
                        case length(DifferenceTypes) > 0 of
                            true ->
                                debug(
                                    "=> * Adding trace to list to explore.~n",
                                    []
                                ),

                                case
                                    lists:keymember(
                                        DifferenceTypes, 3, NewTraces0
                                    )
                                of
                                    true ->
                                        debug(
                                            "=> * Similar trace already exists, ignoring!~n",
                                            []
                                        ),
                                        NewTraces0;
                                    false ->
                    ...
                    case ReturnValue of
                        true ->
                            ...
                            true = ets:insert(
                                ?RESULTS,
                                {Iteration,
                                    {Iteration, FinalTraceLines, Omissions,
                                        true}}
                            ),
                        ...
                        false ->
                            ...
                            case os:getenv("EXIT_ON_COUNTEREXAMPLE") of
                                ...
                                "true" ->
                                    EndTime = os:timestamp(),
                                    Difference = timer:now_diff(
                                        EndTime, StartTime
                                    ),
                                    DifferenceMs = Difference / 1000,
                                    DifferenceSec = DifferenceMs / 1000,
                                    debug(
                                        "Counterexample identified in ~p seconds at iteration: ~p.~n",
                                        [DifferenceSec, CurrentIteration]
                                    ),
                        ...
                        false ->
                            case lists:member(Type, V) of
                                true ->
                                    ...
                                    case
                                        lists:member(SearchType, CandidateTrace)
                                    of
                                        true ->
                                            % debug("=> Present!~n", []),
                                            {true, true};
    ...
    OmissionsFilterFun = fun({Type1, Message1}) ->
        case Type1 of
            pre_interposition_fun ->
                {TracingNode1, InterpositionType1, _OriginNode1,
                    _MessagePayload1} = Message1,
                    ...
                    forward_message ->
                        case TracingNode =:= TracingNode1 of
                            true ->
                                case
                                    lists:member(
                                        element(2, message_type(Message1)),
                                        BackgroundAnnotations
                                    )
                                of
                                    true ->
                        ...
                        true ->
                            ...
                            case LastMessageForTracingNode =:= Line of
                                ...
                                false ->
                                    ...
                                    dict:store(
                                        TracingNode, false, FaultedNodes0
                                    )
                            end;
                        false ->
                            %% Keep faulted.
                        ...
                        true ->
                            case LastMessageForTracingNode =:= Line of
                                true ->
                                    ...
                                    dict:store(
                                        TracingNode, true, FaultedNodes0
                                    );
                                false ->
...
verify_annotations(CausalityAnnotations, MessageTypesFromTraceLines) ->
    Results = lists:foldl(
        fun({MessageType, Requirements}, Acc) ->
            ...
            case IsPresent of
                true ->
                    debug("=> has the following requirements: ~p", [
                        Requirements
                    ]),
                    ...
                    PrefixTrace = lists:sublist(
                        MessageTypesFromTraceLines, 1, Nth
                    ),

                    %% Look at each requirement.
                    PossiblyWrong = lists:foldl(
                        fun(Requirement, Acc1) ->
                            debug(" -> examining requirement: ~p", [Requirement]),

                            case Requirement of
                                true ->
                                    debug(
                                        " -> criteria trivially true, MET!", []
                                    ),
                                    Acc1;
                                {Message, Criteria} ->
                                    %% Find all messages meeting the criteria.
                                    CriteriaMessages = lists:filter(
                                        fun(N) -> N =:= Message end, PrefixTrace
                                    ),
                                    debug(
                                        " -> found ~p messages fitting criteria",
                                        [length(CriteriaMessages)]
                                    ),
                    ...
                    case PossiblyWrong of
                        [] ->
                            debug(
                                "=> at the end of requirements analysis, requirements correct!",
                                []
                            ),
                            Acc;
                        _ ->
                            debug(
                                "=> at the end of requirements analaysis, possible incorrect annotations: ~p",
                                [PossiblyWrong]
                            ),
    ...
    lists:foreach(
        fun({MessageType, Requirements}) ->
            lists:foreach(
                fun({Requirement, Observed}) ->
                    debug(
                        "WARNING: Annotation ~p for ~p potentially too strong based on trace (observed ~p message(s)).",
                        [MessageType, Requirement, Observed]
                    )
                end,
                Requirements
            )
