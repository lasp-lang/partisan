...
%% %CopyrightEnd%
%%
-module(partisan_gen_server_SUITE).
...
start(Config) when is_list(Config) ->
    ...
    %% {ok, Pid4} =
    %% partisan_gen_server:start({global, my_test_name},
    %%          partisan_gen_server_SUITE, [], []),
    %% ok = partisan_gen_server:call({global, my_test_name}, started_p),
    %% {error, {already_started, Pid4}} =
    %% partisan_gen_server:start({global, my_test_name},
    %%          partisan_gen_server_SUITE, [], []),
    %% ok = partisan_gen_server:call({global, my_test_name}, stop),
    %% busy_wait_for_process(Pid4,600),
    %% {'EXIT', {noproc,_}} = (catch partisan_gen_server:call(Pid4, started_p, 10)),

    %% global register linked
    %% >>>>> Commented as partisan doesn't currently support global <<<<<
    %% {ok, Pid5} =
    %% partisan_gen_server:start_link({global, my_test_name},
    %%               partisan_gen_server_SUITE, [], []),
    %% ok = partisan_gen_server:call({global, my_test_name}, started_p),
    ...
    %% >>>>> Commented as partisan doesn't currently support global <<<<<
    %% {ok, {Pid5b, Mon5b}} =
    %% partisan_gen_server:start_monitor({global, my_test_name},
    %%                              partisan_gen_server_SUITE, [], []),
    %% ok = partisan_gen_server:call({global, my_test_name}, started_p),
...
%% --------------------------------------
%% Test partisan_gen_server:call and handle_call.
...
call(Config) when is_list(Config) ->
    ...
    timer:sleep(500),
    ok = partisan_gen_server:call(my_test_name, next_call),
    ok = partisan_gen_server:call(my_test_name, {call_within, 1000}),
    timer:sleep(1500),
...
%% --------------------------------------
%% Test partisan_gen_server:send_request.
%% --------------------------------------
...
send_request(Config) when is_list(Config) ->
    ...
    %% {ok, RPid} = partisan_rpc:call(Remote, partisan_gen_server, start, [{global, remote}, ?MODULE, [], []]),
    ...
    %% {ok, _} = partisan_rpc:call(Remote, partisan_gen_server, start, [{local, remote}, ?MODULE, [], []]),
...
continue(Config) when is_list(Config) ->
    ...
    partisan_gen_server:call(Pid, {continue_reply, self()}),
    [{Ref2, continue}, {Ref2, after_continue}] = read_replies(Pid),
    partisan:is_local_pid(Ref2, Pid) orelse ct:fail(continue),

    partisan_gen_server:call(Pid, {continue_noreply, self()}),
    [{Ref3, continue}, {Ref3, after_continue}] = read_replies(Pid),
    partisan:is_local_pid(Ref3, Pid) orelse ct:fail(continue),

    partisan_gen_server:cast(Pid, {continue_noreply, self()}),
    [{Ref4, continue}, {Ref4, after_continue}] = read_replies(Pid),
    partisan:is_local_pid(Ref4, Pid) orelse ct:fail(continue),

    %% Pid ! {continue_noreply, self()},
    partisan:send(Pid, {continue_noreply, partisan:self()}),
    [{Ref5, continue}, {Ref5, after_continue}] = read_replies(Pid),
    partisan:is_local_pid(Ref5, Pid) orelse ct:fail(continue),
    ...
    %% Pid ! continue_stop,
    partisan:send(Pid, continue_stop),
...
%% --------------------------------------
%% Test partisan_gen_server:cast and handle_cast.
%% Test all different return values from
%% handle_cast.
%% --------------------------------------
...
hibernate(Config) when is_list(Config) ->
    ...
    true = partisan_gen_server:call(my_test_name_hibernate, hibernate),
    is_in_erlang_hibernate(Pid),
    partisan_sys:suspend(my_test_name_hibernate),
    is_in_erlang_hibernate(Pid),
...
auto_hibernate(Config) when is_list(Config) ->
    ...
    is_in_erlang_hibernate(Pid),
    %% Get state test
    State = partisan_sys:get_state(my_test_name_auto_hibernate),
    is_in_erlang_hibernate(Pid),
    %% Call test
    ok = partisan_gen_server:call(my_test_name_auto_hibernate, started_p),
    is_not_in_erlang_hibernate(Pid),
    timer:sleep(HibernateAfterTimeout),
    is_in_erlang_hibernate(Pid),
    %% Cast test
    ...
    %% Info test
    %% Pid ! {self(),handle_info},
    partisan:send(Pid, {partisan:self(), handle_info}),
    receive
        {Pid, handled_info} ->
            ok
    after 1000 ->
    ...
    ok = partisan_gen_server:call(my_test_name_auto_hibernate, stop),
    receive
        {'EXIT', Pid, stopped} ->
            ok
    after 5000 ->
...
%% --------------------------------------
%% Test partisan_gen_server:multicall and handle_call.
%% Test all different return values from
%% handle_call.
%% --------------------------------------
...
multicall_down(Config) when is_list(Config) ->
    ...
    %% We use 'global' as a partisan_gen_server to call.
...
otp_5854(Config) when is_list(Config) ->
    ...
    %% Make sure partisan_gen_server:enter_loop does not accept {local,Name}
    ...
    %% Make sure partisan_gen_server:enter_loop does not accept {global,Name}
    %% when it's another process than the calling one which is
    %% registered under that name
...
%% make sure that the process is not registered when partisan_gen_server:start()
%% returns.
...
call_format_status(Config) when is_list(Config) ->
    ...
    Status2 = partisan_sys:get_status(call_format_status, 5000),
    ...
    {ok, Pid3} = partisan_gen_server:start_link(?MODULE, [], []),
    Status3 = partisan_sys:get_status(Pid3),
    ...
    {ok, Pid4} = partisan_gen_server:start_link(GlobalName1, ?MODULE, [], []),
    Status4 = partisan_sys:get_status(Pid4),
    ...
    {ok, Pid5} = partisan_gen_server:start_link(GlobalName2, ?MODULE, [], []),
    Status5 = partisan_sys:get_status(GlobalName2),
...
replace_state(Config) when is_list(Config) ->
    ...
    NState2 = partisan_sys:replace_state(Pid, Replace2, 5000),
    ...
    NState3 = partisan_sys:replace_state(Pid, Replace4),
    ok = partisan_sys:resume(Pid),
    NState3 = partisan_sys:get_state(Pid, 5000),
    ok.
...
undef_handle_cast(_Config) ->
    {ok, Server} = oc_server:start(),
    MRef = monitor(process, Server),
    partisan_gen_server:cast(Server, cast_msg),
...
undef_in_terminate(Config) when is_list(Config) ->
    ...
    {ok, Server} = partisan_gen_server:start(?MODULE, {state, State}, []),
    try
        ok = partisan_gen_server:stop(Server),
        ct:fail(failed)
    catch
...
fake_upgrade(Pid, Mod) ->
    ...
    partisan_sys:replace_state(Pid, fun(State) -> {new, State} end),
...
start_link(Init, Options) ->
    partisan_proc_lib:start_link(?MODULE, Init, Options).
...
spec_init_default_timeout({ok, Name}, Options) ->
    ...
    partisan_proc_lib:init_ack({ok, self()}),
    %% Supervised init can occur here  ...
    partisan_gen_server:enter_loop(?MODULE, Options, {}, {local, Name}).
...
spec_init_global_default_timeout({ok, Name}, Options) ->
    ...
    partisan_proc_lib:init_ack({ok, self()}),
    %% Supervised init can occur here  ...
    partisan_gen_server:enter_loop(?MODULE, Options, {}, {global, Name}).

spec_init_anonymous(Options) ->
    process_flag(trap_exit, true),
    partisan_proc_lib:init_ack({ok, self()}),
    %% Supervised init can occur here  ...
    partisan_gen_server:enter_loop(?MODULE, Options, {}, infinity).
...
%%% --------------------------------------------------------
%%% Here is the tested partisan_gen_server behaviour.
%%% --------------------------------------------------------
...
handle_info(timeout, {delayed_cast, From}) ->
    %% From ! {self(), delayed},
    partisan:send(From, {partisan:self(), delayed}),
    {noreply, []};
handle_info(timeout, {delayed_info, From}) ->
    %% From ! {self(), delayed_info},
    case partisan:is_local_pid(From) of
        true ->
            partisan:send(From, {self(), delayed_info});
        false ->
            partisan:send(From, {partisan:self(), delayed_info})
    end,
    {noreply, []};
...
handle_info({From, handle_info}, _State) ->
    ...
    case partisan:is_local_pid(From) of
        true ->
            partisan:send(From, {self(), handled_info});
        false ->
            partisan:send(From, {partisan:self(), handled_info})
    end,
    {noreply, []};
...
handle_info({after_continue, Pid}, State) ->
    ...
    %% Pid ! {self(), ack},
    partisan:send(Pid, {partisan:self(), after_continue}),
    partisan:send(Pid, {partisan:self(), ack}),
    {noreply, State};
...
handle_continue({continue, Pid}, State) ->
    ...
    partisan:send(Pid, {partisan:self(), before_continue}),
    self() ! {after_continue, Pid},
    {noreply, State, {continue, {message, Pid}}};
...
handle_continue({message, Pid}, State) ->
    %% Pid ! {self(), continue},
    partisan:send(Pid, {partisan:self(), continue}),
    {noreply, State};
...
terminate({From, stopped}, _State) ->
    io:format("FOOBAR"),
    %% From ! {self(), stopped},
    partisan:send(From, {partisan:self(), stopped}),
    ok;
terminate({From, stopped_info}, _State) ->
    %% From ! {self(), stopped_info},
    case partisan:is_local_pid(From) of
        true ->
            partisan:send(From, {self(), stopped_info});
        false ->
            partisan:send(From, {partisan:self(), stopped_info})
    end,
    ok;
