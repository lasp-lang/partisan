...
%% %CopyrightEnd%
%%
-module(partisan_gen_statem_SUITE).
...
start4(Config) ->
    ...
    ignore = partisan_gen_statem:start(?MODULE, start_arg(Config, ignore), []),
...
sys1(Config) ->
    ...
    %% {timeout,_} =
    %%  ?EXPECT_FAILURE(partisan_gen_statem:call(Pid, hej), Reason),
...
hibernate(Config) ->
    ...
    hibernating = partisan_gen_statem:call(Pid, hibernate_sync),
    wait_erlang_hibernate(Pid),
    please_just_five_more = partisan_gen_statem:call(Pid, snooze_sync),
    wait_erlang_hibernate(Pid),
    good_morning = partisan_gen_statem:call(Pid, wakeup_sync),
    is_not_in_erlang_hibernate(Pid),
    ...
    ok = partisan_gen_statem:cast(Pid, hibernate_async),
    wait_erlang_hibernate(Pid),
    ok = partisan_gen_statem:cast(Pid, snooze_async),
    wait_erlang_hibernate(Pid),
    ok = partisan_gen_statem:cast(Pid, wakeup_async),
    is_not_in_erlang_hibernate(Pid),
...
auto_hibernate(Config) ->
    ...
    wait_erlang_hibernate(Pid),
    %% After cast test
    ok = partisan_gen_statem:cast(Pid, {hping, self()}),
    receive
        {Pid, hpong} ->
            ok
    after 1000 ->
    ...
    wait_erlang_hibernate(Pid),
    %% After call test
    hpong = partisan_gen_statem:call(Pid, hping),
    is_not_in_erlang_hibernate(Pid),
    timer:sleep(HibernateAfterTimeout),
    wait_erlang_hibernate(Pid),
    %% Timer test 1
    TimerTimeout1 = HibernateAfterTimeout div 2,
    ok = partisan_gen_statem:call(Pid, {start_htimer, self(), TimerTimeout1}),
    is_not_in_erlang_hibernate(Pid),
    ...
    ok = partisan_gen_statem:call(Pid, {start_htimer, self(), TimerTimeout2}),
    is_not_in_erlang_hibernate(Pid),
    timer:sleep(HibernateAfterTimeout),
    wait_erlang_hibernate(Pid),
    receive
...
enter_loop(_Config) ->
    ...
    %% Make sure partisan_gen_statem:enter_loop does not accept {local,Name}
    ...
    %% Make sure partisan_gen_statem:enter_loop does not accept {global,Name}
    %% when it's another process than the calling one which is
    %% registered under that name
...
reply_by_alias_with_payload(Config) when is_list(Config) ->
    ...
    %% partisan_gen_statem:reply/2 works with it...
    %%
    %% Whitebox...
    Reply = make_ref(),
    Alias = alias(),
