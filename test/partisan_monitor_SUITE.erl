...
expect_down(Ref, P) ->
    receive
        {'DOWN', PRef, process, PP, Reason} = Signal ->
            (PRef =:= Ref orelse partisan:is_local_reference(PRef, Ref))
                andalso (
                    PP =:= P
                    orelse (
                        is_pid(P)
                        andalso partisan:is_local_pid(PP, P)
                    )
                    orelse (
                        is_pid(PP)
                        andalso partisan:is_local_pid(P, PP)
                    )
                    orelse (
                        is_tuple(P)
                        andalso P =:= partisan_remote_ref:to_term(PP)
                    )
                )
                orelse ct:fail([{rec, Signal}, {args, [Ref, P]}]),
            Reason;
        Other ->
    ...
    receive
        {'DOWN', PRef, process, PP, Reason} = Signal ->
          (PRef =:= Ref orelse partisan:is_local_reference(PRef, Ref))
                ...
                orelse ct:fail([{rec, Signal}, {args, [Ref, P, Reason]}]),
            ok;
        Other ->
...
down_on_alias_gh5310_test(ImmedExitReason, DeMonSched, TermSched) ->
    ...
    DeMonPid = spawn_opt(fun () ->
                                 AliasMon = partisan:monitor(process, TermPid,
    ...
    receive
        ...
        {Parent, {monitor_process, P}} ->
            Parent ! {self(), {monitor_process, 
                               catch partisan:monitor(process, P) }},
            jeeves_loop(Parent);
