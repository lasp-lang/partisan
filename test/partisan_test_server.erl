...
handle_call(delayed_reply_call, From, State) ->
    ct:pal("Received delayed_reply_call message from ~p in the handle_call handler.", [From]),
    partisan_gen_server:reply(From, ok),
    {noreply, State};
handle_call(call, From, State) ->
    ct:pal("Received call message from ~p in the handle_call handler.", [From]),
    {reply, ok, State};
...
handle_cast({cast, ServerRef}, State) ->
    ct:pal("Received cast message with server_ref: ~p in the handle_call handler.", [ServerRef]),
    ServerRef ! ok,
    {noreply, State};
