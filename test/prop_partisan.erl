...
finite_fault_commands(Module) ->
    ?LET(
        ...
        begin
            ...
            CommandsWithoutGlobalNodeCommands = lists:filter(
                fun({set, {var, _Nth}, {call, _Mod, Fun, _Args}}) ->
            ...
            CommandsWithOnlyGlobalNodeCommandsWithoutAssertions = lists:map(
                fun(Fun) ->
                    {set, {var, 0}, {call, system_model(), Fun, []}}
                end,
                node_global_functions() -- node_assertion_functions()
            ),

            %% Only global node commands with global assertions.
            CommandsWithOnlyGlobalNodeCommandsAssertionsOnly = lists:map(
                fun(Fun) ->
                    {set, {var, 0}, {call, system_model(), Fun, []}}
                end,
                sets:to_list(
                    sets:intersection(
                        sets:from_list(node_assertion_functions()),
                        sets:from_list(node_global_functions())
                    )
                )
            ),
            ...
            {FinalCommands, _} = lists:foldl(
                fun({set, {var, _Nth}, {call, Mod, Fun, Args}}, {Acc, Next}) ->
                    {
                        Acc ++ [{set, {var, Next}, {call, Mod, Fun, Args}}],
                        Next + 1
                    }
                end,
                {[], 1},
...
single_success_commands(Module) ->
    ?LET(
        ...
        begin
            ...
            CommandsWithoutGlobalNodeCommandsWithoutAssertions = lists:filter(
                fun({set, {var, _Nth}, {call, _Mod, Fun, _Args}}) ->
            ...
            FirstNonGlobalCommand =
                case
                    length(CommandsWithoutGlobalNodeCommandsWithoutAssertions) >
                        0
                of
                    true ->
                        [
                            hd(
                                CommandsWithoutGlobalNodeCommandsWithoutAssertions
                            )
                        ];
                    false ->
                        []
            ...
            CommandsWithOnlyGlobalNodeCommands = lists:map(
                fun(Fun) ->
                    {set, {var, 0}, {call, system_model(), Fun, []}}
                end,
            ...
            %% Derive final command sequence.
            FinalCommands0 =
                case os:getenv("IMPLEMENTATION_MODULE", "false") of
                    _ ->
                        lists:flatten(
...
initial_state() ->
    ...
    %% Debug message.
    initial_state_debug("initial_state: nodes ~p joined_nodes ~p", [
        Nodes, JoinedNodes
    ]),
...
command(State) ->
    ...
    FaultModelCommands = lists:flatmap(
        fun(Command) ->
            case fault_injection_enabled() of
                true ->
                    [{1, Command}];
...
precondition(
    #property_state{nodes = Nodes, joined_nodes = JoinedNodes},
    {call, _Mod, sync_join_cluster, [Node, JoinNode]}
) ->
    %% Only allow dropping of the first unjoined node in the nodes list, for ease of debugging.
    precondition_debug(
        "precondition sync_join_cluster: invoked for node ~p joined_nodes ~p", [
            Node, JoinedNodes
        ]
    ),
    ...
    precondition_debug(
        "precondition sync_join_cluster: remaining nodes to be joined are: ~p",
        [CanBeJoinedNodes]
    ),
    ...
    case length(CanBeJoinedNodes) > 0 of
        true ->
            precondition_debug(
                "precondition sync_join_cluster: attempting to join ~p", [Node]
            ),

            case lists:member(Node, CanBeJoinedNodes) of
                true ->
                    precondition_debug(
                        "precondition sync_join_cluster: POSSIBLE YES attempting to join ~p is in ~p",
                        [Node, CanBeJoinedNodes]
                    ),
                    lists:member(JoinNode, JoinedNodes);
                _ ->
                    precondition_debug(
                        "precondition sync_join_cluster: NO attempting to join ~p not in ~p",
                        [Node, CanBeJoinedNodes]
                    ),
                    false
            end;
        false ->
            precondition_debug(
                "precondition sync_join_cluster: no nodes left to join.", []
            ),
            %% Might need to be changed when there's no read/write operations.
            false
    end;
precondition(
    #property_state{joined_nodes = JoinedNodes},
    {call, _Mod, sync_leave_cluster, [Node]}
) ->
    %% Only allow dropping of the last node in the join list, for ease of debugging.
    precondition_debug(
        "precondition sync_leave_cluster: invoked for node ~p joined_nodes ~p",
        [Node, JoinedNodes]
    ),

    ToBeRemovedNodes = JoinedNodes,
    precondition_debug(
        "precondition sync_leave_cluster: remaining nodes to be removed are: ~p",
        [ToBeRemovedNodes]
    ),
    ...
    case length(ToBeRemovedNodes) > 3 of
        true ->
            ...
            precondition_debug(
                "precondition sync_leave_cluster: attempting to leave ~p", [
                    Node
                ]
            ),
            ...
            case lists:member(Node, CanBeRemovedNodes) of
                true ->
                    precondition_debug(
                        "precondition sync_leave_cluster: YES attempting to leave ~p is in ~p",
                        [Node, CanBeRemovedNodes]
                    ),
                    true;
                _ ->
                    precondition_debug(
                        "precondition sync_leave_cluster: NO attempting to leave ~p is not in ~p",
                        [Node, CanBeRemovedNodes]
                    ),
                    false
            end;
        false ->
            precondition_debug(
                "precondition sync_leave_cluster: no nodes left to remove.", []
            ),
            false
    end;
precondition(
    ...
    precondition_debug(
        "precondition fired for counter ~p and node function: ~p(~p)", [
            Counter, Fun, Args
        ]
    ),
    ...
    case lists:member(Fun, node_functions()) of
        true ->
            ClusterCondition =
                enough_nodes_connected(JoinedNodes) andalso
                    is_joined(Node, JoinedNodes),
            ...
            case lists:member(Fun, fault_functions(JoinedNodes)) of
                true ->
                    ClusterCondition =
                        enough_nodes_connected(JoinedNodes) andalso
                            is_joined(Node, JoinedNodes),
                    FaultModelPrecondition = fault_precondition(
                        FaultModelState, Call
                    ),
                    ClusterCondition andalso FaultModelPrecondition;
                false ->
                    debug(
                        "general precondition fired for mod ~p and fun ~p and args ~p",
                        [Mod, Fun, Args]
                    ),
                    false
            end
...
precondition(#property_state{}, {call, _Mod, forced_failure, _Args}) ->
    ...
    true;
precondition(
    #property_state{counter = Counter}, {call, _Mod, Fun, Args} = _Call
) ->
    precondition_debug(
        "fallthrough precondition fired for counter ~p and node function: ~p(~p)",
        [Counter, Fun, Args]
    ),
...
next_state(
    #property_state{counter = Counter, joined_nodes = JoinedNodes} = State,
    _Res,
    {call, ?MODULE, sync_join_cluster, [Node, _JoinNode]}
) ->
    State#property_state{
        joined_nodes = JoinedNodes ++ [Node], counter = Counter + 1
    };
next_state(
    #property_state{counter = Counter, joined_nodes = JoinedNodes} = State,
    _Res,
    {call, ?MODULE, sync_leave_cluster, [Node]}
) ->
    State#property_state{
        joined_nodes = JoinedNodes -- [Node], counter = Counter + 1
    };
next_state(
    #property_state{} = State, _Res, {call, ?MODULE, forced_failure, []}
) ->
    State;
next_state(
    #property_state{
        counter = Counter,
        fault_model_state = FaultModelState0,
        node_state = NodeState0,
        joined_nodes = JoinedNodes
    } = State,
    Res,
    {call, _Mod, Fun, _Args} = Call
) ->
    case lists:member(Fun, node_functions()) of
        true ->
        ...
        false ->
            case lists:member(Fun, fault_functions(JoinedNodes)) of
                true ->
                    FaultModelState = fault_next_state(
                        State, FaultModelState0, Res, Call
                    ),
                    State#property_state{
                        fault_model_state = FaultModelState,
                        counter = Counter + 1
...
postcondition(
    _State,
    {call, ?MODULE, sync_join_cluster, [_Node, _JoinNode]},
    {ok, _Members}
) ->
    postcondition_debug("postcondition sync_join_cluster fired", []),
    true;
postcondition(
    _State, {call, ?MODULE, sync_leave_cluster, [_Node]}, {ok, _Members}
) ->
...
postcondition(
    ...
    {call, Mod, Fun, [_Node | _] = Args} = Call,
    Res
) ->
    case lists:member(Fun, node_functions()) of
        true ->
            ...
            case PostconditionResult of
                false ->
                    debug("postcondition result: ~p; command: ~p:~p(~p)", [
                        PostconditionResult, Mod, Fun, Args
                    ]),
                    ok;
                true ->
                    ok
        ...
        false ->
            case lists:member(Fun, fault_functions(JoinedNodes)) of
                true ->
                    PostconditionResult = fault_postcondition(
                        FaultModelState, Call, Res
                    ),

                    case PostconditionResult of
                        false ->
                            debug(
                                "postcondition result: ~p; command: ~p:~p(~p)",
                                [PostconditionResult, Mod, Fun, Args]
                            ),
                            ok;
                        true ->
                ...
                false ->
                    postcondition_debug(
                        "general postcondition fired for ~p:~p with response ~p",
                        [Mod, Fun, Res]
                    ),
...
postcondition(
    #property_state{node_state = NodeState}, {call, _Mod, Fun, Args} = Call, Res
) ->
    postcondition_debug(
        "fallthrough postcondition fired node function: ~p(~p)", [Fun, Args]
    ),
...
command_preamble(Node, Command) ->
    ...
    partisan_trace_orchestrator:trace(enter_command, {
        normalize_name(Node), Command
    }),

    %% Under replay, perform the trace replay.
    partisan_trace_orchestrator:replay(enter_command, {
        normalize_name(Node), Command
    }),
...
command_conclusion(Node, Command) ->
    ...
    %% Log command entrance trace.
    partisan_trace_orchestrator:trace(exit_command, {
        normalize_name(Node), Command
    }),

    %% Under replay, perform the trace replay.
    partisan_trace_orchestrator:replay(exit_command, {
        normalize_name(Node), Command
    }),
...
sync_join_cluster(Node, JoinNode) ->
    ...
    debug(
        "sync_join_cluster: joining node ~p to cluster at node ~p with current members ~p",
...
sync_leave_cluster(Node) ->
    ...
    debug(
        "sync_leave_cluster: leaving node ~p from cluster with current members ~p",
        [?NAME(Node), Members]
    ),
...
start_or_reload_nodes() ->
    ...
    Nodes =
        case RunningNodes =/= [] andalso not restart_nodes() of
            false ->
                debug("starting instances, restart_nodes => ~p", [
                    restart_nodes()
                ]),
            ...
            true ->
                debug(
                    "not starting instances, restart_nodes => ~p, running_nodes; ~p",
                    [restart_nodes(), RunningNodes]
                ),
                ...
                case full_restart() of
                    true ->
                        ...
                        lists:foreach(
                            fun({ShortName, _}) ->
                                ok = rpc:call(
                                    ?NAME(ShortName), application, stop, [
                                        partisan
                                    ]
                                ),
                                {ok, _} = rpc:call(
                                    ?NAME(ShortName),
                                    applicatioShortNamen,
                                    ensure_all_started,
                                    [partisan]
                                )
                            end,
                        ...
                        debug("reclustering nodes took ~p ms", [
                            ClusterTime / 1000
                        ]),
    ...
    PreInterpositionFun = fun({Type, OriginNode, OriginalMessage}) ->
        ...
        ok = partisan_trace_orchestrator:replay(pre_interposition_fun, {
            node(), Type, OriginNode, OriginalMessage
        }),

        %% Record message incoming and outgoing messages.
        ok = partisan_trace_orchestrator:trace(pre_interposition_fun, {
            node(), Type, OriginNode, OriginalMessage
        }),
    ...
    lists:foreach(
        fun({_Name, Node}) ->
            rpc:call(Node, ?MANAGER, add_pre_interposition_fun, [
    ...
    PostInterpositionFun = fun(
        {Type, OriginNode, OriginalMessage},
        {Type, OriginNode, RewrittenMessage}
    ) ->
        %% Record outgoing message after transformation.
        ok = partisan_trace_orchestrator:trace(post_interposition_fun, {
            node(), OriginNode, Type, OriginalMessage, RewrittenMessage
        })
    end,
    lists:foreach(
        fun({_Name, Node}) ->
            rpc:call(Node, ?MANAGER, add_post_interposition_fun, [
...
wait_until_nodes_agree_on_membership(Nodes) ->
    AgreementFun = fun(Node) ->
        ...
        case SortedNodes =:= SortedMembers of
            true ->
                debug(
                    "node ~p agrees on membership: ~p",
                    [Node, SortedMembers]
                ),
                true;
            false ->
                debug(
                    "node ~p disagrees on membership: ~p != ~p",
