...
%% Precondition.
node_precondition(
    _NodeState, {call, ?MODULE, submit_transaction, [_Node, _Message]}
) ->
    true;
...
node_next_state(
    _State,
    #node_state{messages = Messages} = NodeState,
    _Response,
    {call, ?MODULE, submit_transaction, [_Node, Message]}
) ->
...
node_postcondition(
    _NodeState, {call, ?MODULE, trigger_sync, [_Node1, _Node2]}, ok
) ->
    true;
node_postcondition(
    _NodeState, {call, ?MODULE, submit_transaction, [_Node, _Message]}, _Result
) ->
    true;
...
node_postcondition(_NodeState, {call, ?MODULE, check, []}, undefined) ->
    true;
node_postcondition(
    #node_state{messages = Messages} = _NodeState,
    {call, ?MODULE, check, []},
    Chains
) ->
    %% Get pubkey.
    ...
    [{initial_messages, InitialMessages}] = ets:lookup(
        prop_partisan, initial_messages
    ),
    ...
    lists:foreach(
        fun(Chain) ->
            ...
            BlockTxns = lists:flatten([
                partisan_hbbft_worker:block_transactions(B)
             || B <- Chain
            ]),
            ...
            true = sets:is_subset(
                sets:from_list(BlockTxns),
                sets:from_list(Messages ++ InitialMessages)
            ),
            ...
            case
                length(BlockTxns) =:=
                    length(Messages ++ InitialMessages) - sets:size(StillInBuf)
            of
                true ->
                    ok;
                false ->
    ...
    Result =
        OneChain andalso NonTrivialLength andalso BufferEmpty andalso
            Crashed =< Tolerance,
        ...
        [{at_least_one_transaction, true}] ->
            ...
            wait_until(
                fun() ->
                    ...
                    node_debug("message_queue_lens(Workers): ~p should = 0", [
                        message_queue_lens(Workers)
                    ]),
                    node_debug("sets:size(Chains): ~p should = 1", [
                        sets:size(Chains)
                    ]),
                    node_debug(
                        "length(hd(sets:to_list(Chains))): ~p should /= 0", [
                            length(hd(sets:to_list(Chains)))
                        ]
                    ),
...
node_begin_case() ->
    ...
    lists:foreach(
        fun({ShortName, _}) ->
            ...
            {ok, _} = rpc:call(
                ?NAME(ShortName), application, ensure_all_started, [hbbft]
            )
        end,
        Nodes
    ),
    ...
    _ = lists:map(
        fun(Node) -> rpc:call(Node, erlang, load_module, [Mod, Bin]) end, Nodes
    ),

    %% start a hbbft_worker on each node
    Workers = lists:map(
        fun({I, {{Name1, _} = FullName, SK}}) ->
            {ok, Worker} = rpc:call(
                ?NAME(Name1), partisan_hbbft_worker, start_link, [
                    N, F, I, tpke_privkey:serialize(SK), BatchSize, false
                ]
            ),
    ...
    case os:getenv("BOOTSTRAP") of
        "true" ->
            ...
            NumMsgs =
                case os:getenv("BOOTSTRAP_MESSAGES") of
                    false ->
                        N * BatchSize;
                    Other ->
                ...
                fun(Msg) ->
                    ...
                    lists:foreach(
                        fun({_Node, {ok, Worker}}) ->
                            ...
                            partisan_hbbft_worker:submit_transaction(
                                Msg, Worker
                            )
                        end,
                        Workers
            ...
            case
                wait_until(
                    fun() ->
                        ...
                        % node_debug("Chains: ~p", [sets:to_list(Chains)]),
                        node_debug(
                            "message_queue_lens(Workers): ~p should = 0", [
                                message_queue_lens(Workers)
                            ]
                        ),
                        node_debug("sets:size(Chains): ~p should = 1", [
                            ...
                            true ->
                                lists:foreach(
                                    fun(X) ->
                                        ...
                                        Chain = lists:nth(
                                            X, sets:to_list(Chains)
                                        ),

                                        %% check all transactions are unique
                                        node_debug("=> number of blocks: ~p", [
                                            length(Chain)
                                        ]),
                                        BlockTxns = lists:flatten([
                                            partisan_hbbft_worker:block_transactions(
                                                B
                                            )
                                         || B <- Chain
                                        ]),
                                    ...
                                    lists:seq(1, length(sets:to_list(Chains)))
                                ),
            ...
            lists:foreach(
                fun(Chain) ->
                    ...
                    true =
                        length(BlockTxns) ==
                            sets:size(sets:from_list(BlockTxns)),

                    %% check they're all members of the original message list
                    true = sets:is_subset(
                        sets:from_list(BlockTxns), sets:from_list(Msgs)
                    ),
                    node_debug("chain contains ~p distinct transactions~n", [
                        length(BlockTxns)
                    ])
                end,
                sets:to_list(Chains)
...
node_end_case() ->
    ...
    [{workers, Workers}] = ets:lookup(prop_partisan, workers),
    lists:foreach(
        fun({_, {ok, W}}) -> catch partisan_hbbft_worker:stop(W) end, Workers
    ),
    ok = global:sync(),
...
message_queue_lens(Workers) ->
    Values = lists:map(
        fun({{Name1, _}, {ok, W}}) ->
            try
                Result = rpc:call(?NAME(Name1), erlang, process_info, [
                    W, message_queue_len
                ]),
                element(2, Result)
            catch
            ...
            fun({_Node, {ok, W}}, Acc) ->
                ...
                try
                    ...
                    #{
                        ...
                        round := Round,
                        acs := #{
                            completed_bba_count := BBAC,
                            successful_bba_count := BBAS,
                            completed_rbc_count := RBCC
                        }
                    } = Status,
                    node_debug(
                        "=> Status: rbc completed: ~p bba completed ~p bba successful ~p Buffer size ~p Round ~p",
                        [RBCC, BBAC, BBAS, Buf, Round]
                    ),
                    Acc ++ [Status]
                catch
                    _:Error ->
