...
node_postcondition(
    _NodeState, {call, ?MODULE, broadcast, [_Node, _Message]}, {badrpc, timeout}
) ->
    ?LOG_INFO(#{
        description =>
            "Broadcast error with timeout, must have been synchronous!"
    }),
    false;
node_postcondition(
    _NodeState, {call, ?MODULE, broadcast, [_Node, _Message]}, error
) ->
    ?LOG_INFO(#{description => "Broadcast error, must have been synchronous!"}),
    true;
node_postcondition(
    _NodeState, {call, ?MODULE, broadcast, [_Node, _Message]}, ok
) ->
    true;
node_postcondition(
    #node_state{failed_to_send = FailedToSend, sent = Sent},
    {call, ?MODULE, check_mailbox, []},
    Results
) ->
    AllSentAndReceived = lists:foldl(
        fun({Message, Recipients}, MessageAcc) ->
            node_debug("verifying that message ~p was received by ~p", [
                Message, Recipients
            ]),

            All = lists:foldl(
                fun(Recipient, NodeAcc) ->
                    node_debug(
                        "=> verifying that message ~p was received by ~p", [
                            Message, Recipient
                        ]
                    ),
                    ...
                    case lists:member(Message, Messages) of
                        ...
                        false ->
                            node_debug(
                                "=> => message not received at node ~p: ~p", [
                                    Message, Recipient
                                ]
                            ),
    ...
    FailedToSendNotReceived = lists:foldl(
        fun({Message, Recipients}, MessageAcc) ->
            node_debug("verifying that message ~p was NOT received by ~p", [
                Message, Recipients
            ]),
            ...
            All = lists:foldl(
                fun(Recipient, NodeAcc) ->
                    node_debug(
                        "=> verifying that message ~p was NOT received by ~p", [
                            Message, Recipient
                        ]
                    ),
        ...
        fun(Node, Dict) ->
            ...
            after 10000 ->
                case rpc:call(?NAME(Node), erlang, is_process_alive, []) of
                    ...
                    Other ->
                        node_debug(
                            "=> no response, asked if node was alive: ~p", [
                                Other
                            ]
                        ),
...
node_begin_case() ->
    ...
    lists:foreach(
        fun({ShortName, _}) ->
            ...
            node_debug(
                "spawning broadcast receiver on node ~p, our pid is: ~p", [
                    ShortName, Self
                ]
            ),
            ...
            RemoteFun = fun() ->
                ...
                ReceiverFun = fun(F) ->
                    receive
                        {received, Sender} ->
                            Received = ets:foldl(
                                fun(Term, Acc) -> Acc ++ [Term] end, [], ?TABLE
                            ),
                            Sorted = lists:keysort(1, Received),
                            node_debug(
                                "node ~p received request for stored values: ~p",
                                [ShortName, Sorted]
                            ),
                            Sender ! Sorted,
                            F(F);
                        {Id, SourceNode, Value} ->
                            node_debug(
                                "node ~p received origin: ~p id ~p and value: ~p",
                                [ShortName, SourceNode, Id, Value]
                            ),
                            true = ets:insert(?TABLE, {Id, SourceNode, Value}),
                            F(F);
                        terminate ->
                            node_debug("node ~p terminating receiver", [
                                ShortName
                            ]),
                            ok;
                        Other ->
                            node_debug("node ~p received other: ~p", [
                                ShortName, Other
                            ]),
                            F(F)
                    end
...
node_end_case() ->
    ...
    lists:foreach(
        fun({ShortName, _}) ->
            %% node_debug("starting ~p at node ~p with node list ~p ", [?BROADCAST_MODULE, ShortName, SublistNodeProjection]),
            Pid = rpc:call(?NAME(ShortName), erlang, whereis, [
                broadcast_module()
            ]),
            node_debug("process is running on node ~p with id ~p~n", [
                ShortName, Pid
            ]),
    ...
    lists:foreach(
        fun({ShortName, _}) ->
            terminate = rpc:call(?NAME(ShortName), erlang, send, [
                ?RECEIVER, terminate
            ])
        end,
        Nodes
