...
%%% Start child
start_child() ->
    partisan_gen_server:start_link({local, ?MODULE}, ?MODULE, [child], []).

start_child_noreg() ->
    partisan_gen_server:start_link(?MODULE, [child], []).

restart_child() ->
    partisan_gen_server:cast(supervisor_deadlock, restart).

restart_child(Pid) ->
    partisan_gen_server:cast(Pid, restart).